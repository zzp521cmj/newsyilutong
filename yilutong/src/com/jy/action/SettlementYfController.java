 ...
 * */
@Controller
@RequestMapping(value = "settlementYf.do")
public class SettlementYfController {
	
	@Resource
	private SettlementYfService bid;
	@Resource
	...
	public String getSettlementYfSearchPage(HttpServletRequest request,String pid){
		request.setAttribute("pid", pid);
		return "settlementYf/settlementYfInfo";
	}
...
//		System.out.println(json.toString());
	...
	//审核信息保存
		public List<SettlyfSh> saveSettlyfSh(String [] orderid,String checks_type,String checks_notes,String checks_state,String checks_user,String  settlyf_sf,List<Settle_history>listSet ,String did) {
			...
			List<ShippingOrder> order= bid.getSettlementYfout(null, null, null, null, null, null, null, null, null, null,orderid);	
	...
	public Map<String,Object> saveSettlementYf(String[] dds,String type, String remarks,String khbank,String usernames,String kahao,String zpnum,String userid,String state) {	
		...
		for(int i=0;i<order.size();i++){
			...
			if(order.get(i).getPaytypes().equals("0")){
				sh.setAllmoney(order.get(i).getTransport_pay());				
			}else{
				sh.setAllmoney(Float.parseFloat(order.get(i).getTransport_pay())-Float.parseFloat(order.get(i).getSettlyf_fu().split(",")[1])+"" );
			}
			sh.setOrder_id(order.get(i).getShiping_order_id());	
			if(order.get(i).getPaytypes().equals("0")){
				sh.setSettl_money(order.get(i).getReality_sum());				
			}else{
				sh.setSettl_money(Float.parseFloat(order.get(i).getReality_sum())-Float.parseFloat(order.get(i).getSettlyf_fu().split(",")[0])+"");
			}
			...
			list.add(sh);
			if(order.get(i).getPaytypes().equals("0")){
				order.get(i).setSettlyf_fu(order.get(i).getReality_sum()+","+0);		
			}else{
				order.get(i).setSettlyf_fu(order.get(i).getReality_sum()+","+order.get(i).getSettlyf_fu().split(",")[0]);
			}
	...
	Json addSettlementYf(String[] dds,String type, String remarks,String khbank,String usernames,String kahao,
		...
		Json json = new Json();
		//保存结算记录
		bid.saveSettlchecks((List<Settle_history>) saveSettlementYf(dds, type, remarks, khbank, usernames, kahao, zpnum, user.getId(),"0").get("list"));
	...
	Json delSettlementYf(String[] dds,String did,String notes,HttpServletRequest request) {
		...
		List<Settle_history> list=sid.getupdateSettle("jy_settlementy_history", 0, dds);
		sid.updateSettle(list,"jy_settlementy_history","2");
		bid.saveSettlyfSh(saveSettlyfSh(dds, "0", notes,"2", user.getId(),"0",list,did));		
		if (truck > 0) {
...
//			System.out.println(json.toString());
	...
	Json shSettlementYfTG(String[] ids,String notes,HttpServletRequest request) {
		User user=(User) request.getSession().getAttribute(SessionInfo.SessionName);
		Json json = new Json();
		List<Settle_history> list=sid.getupdateSettle("jy_settlementy_history",0, ids);
		sid.updateSettle(list,"jy_settlementy_history","2");
		bid.saveSettlyfSh(saveSettlyfSh(ids, "1", notes,"1", user.getId(),"0",list,user.getDid()));
	...
	Json shSettlementYfBH(String[] ids,String notes,HttpServletRequest request) {
		...
		bid.saveSettlyfSh(saveSettlyfSh(ids, "1", notes,"0", user.getId(),"0",list,user.getDid()));
		List<ShippingOrder> order= bid.getSettlementYfout(null, null, null, null, null, null, null, null, null, null,ids);	
		for(int i=0;i<order.size();i++){			
				order.get(i).setSettlyf_fu(order.get(i).getSettlyf_fu().split(",")[1]+","+order.get(i).getSettlyf_fu().split(",")[1]);
		}
...
//				System.out.println(json.toString());
			...
			public @ResponseBody List<ShippingOrder> getSettlementYfout(HttpServletResponse response,String[] headtitle, String[] fieldName,String ddId, String types, String isdelivery, String start_date,
				...
				try {			
					...
					// 调用工具类创建表头
					ExportUtils.outputHeaders(headtitle, workbook, "运费结算信息导出");
					// 调用工具类生成内容
					ExportUtils.outputColums(fieldName, list, workbook, 1, "运费结算信息导出");
...
//					System.out.println(json.toString());
			...
			public @ResponseBody List<ShippingOrder> getCustomerSetYfout(HttpServletResponse response,String[] headtitle, String[] fieldName,String customer_name, String customer_num, String [] checkarray,String starts, String ends){
				...
				try {			
					...
					OutputStream ouputStream = response.getOutputStream();
					// 调用工具类创建表头
					ExportUtils.outputHeaders(headtitle, workbook, "客户运费结算信息导出");
					// 调用工具类生成内容
					ExportUtils.outputColums(fieldName, list, workbook, 1, "客户运费结算信息导出");
					workbook.write(ouputStream);
					ouputStream.flush();
					ouputStream.close();
...
//			查询
			...
			public @ResponseBody List<ShippingOrder> getSettlementYfSHout(HttpServletResponse response,String[] headtitle, String[] fieldName,String ddId, String types, String isdelivery, String start_date,
				...
				try {			
					...
					OutputStream ouputStream = response.getOutputStream();
					// 调用工具类创建表头
					ExportUtils.outputHeaders(headtitle, workbook, "运费结算审核信息导出");
					// 调用工具类生成内容
					ExportUtils.outputColums(fieldName, list, workbook, 1, "运费结算审核信息导出");
					workbook.write(ouputStream);
					ouputStream.flush();
					ouputStream.close();
