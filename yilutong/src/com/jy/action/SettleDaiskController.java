 ...
 * */
@Controller
@RequestMapping(value = "settleDaisk.do")
public class SettleDaiskController {
...
//		System.out.println(json.toString());
	...
	Json addSettlementYf(String[] dds,String type, String remarks,String khbank,String usernames,String kahao,
		...
		Json json = new Json();
		//保存结算记录
		bid.saveSettlchecks((List<Settle_history>) saveSettlementYf(dds, type, remarks, khbank, usernames, kahao, zpnum, user.getId(),"2").get("list"));
		bid.saveSettl_fu((List<ShippingOrder>) saveSettlementYf(dds, type, remarks, khbank, usernames, kahao, zpnum, user.getId(),"2").get("order"));		
	...
	Json delSettlementYf(String[] dds,String did,String notes,HttpServletRequest request) {
		...
		List<Settle_history> list=bid.getupdateSettle("jy_settlementy_history", 2, dds);//最新结算记录
		bid.updateSettle(list,"jy_settlementy_history","1");//修改结算记录审核状态
		bid.saveSettlyfSh(saveSettlyfSh(dds, "0", notes,"2", user.getId(),"2",list,did));		
		if (truck > 0) {			
			json.setFlag(true);
			return json;
		}
...
//			System.out.println(json.toString());
	...
	Json shSettlementYfTG(String[] ids,String notes,HttpServletRequest request) {
		User user=(User) request.getSession().getAttribute(SessionInfo.SessionName);
		Json json = new Json();
		List<Settle_history> list=bid.getupdateSettle("jy_settlementy_history", 2, ids);
		bid.updateSettle(list,"jy_settlementy_history","2");
		bid.saveSettlyfSh(saveSettlyfSh(ids, "1", notes,"1", user.getId(),"2",list,user.getDid()));
	...
	Json shSettlementYfBH(String[] ids,String notes,HttpServletRequest request) {
		...
		Json json = new Json();
		List<Settle_history> list=bid.getupdateSettle("jy_settlementy_history", 2, ids);
		bid.updateSettle(list,"jy_settlementy_history","3");
		bid.saveSettlyfSh(saveSettlyfSh(ids, "1", notes,"0", user.getId(),"2",list,user.getDid()));
