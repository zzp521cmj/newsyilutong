...
<html>
  ...
  <%String did=user.getCostomersid(); %>
  ...
  <script type="text/javascript">
 		...
 		var pid='<%=flg%>';
  		$(function(){
  			grid=$("#dg").datagrid({
  			url : 'delivery.do?method=gettransferGoods&customid='+cid,
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
  			...
  			columns : [[ 				
				...
				{field : 'send_time',title : '发车日期',width : 120,align:'center'},
  				...
  				{field : 'orderState',title : '运单号',width : 80,align:'center',hidden:true},
  				...
  				{field : 'company_time',title : '中转日期',width : 120,align:'center'},
  				{field : 'company_name',title : '中转公司',width : 80,align:'center'},
  				{field : 'change_pay',title : '应付中转费',width : 80,align:'center'},
  				{field : 'writeoff',title : '付款状态',width : 80,align:'center',hidden:true},
  				{field : 'write_off',title : '付款状态',width : 80,align:'center',
  							formatter:function(val,row,index){
  						if(val==3){
  							return "未付款";
  						}else if(val==0){
  				...
  				{field : 'create_user',title : '中转操作人',width : 80,align:'center'},
  				{field : 'sign_user',title : '签收人',width : 80,align:'center'},
  				{field : 'sign_time',title : '签收日期',width :120,align:'center'},
  				{field : 'sign_user',title : '签收操作人',width :80,align:'center'},
  	...
  	*/
  		...
  		$("#carnum").select2({
			...
			query: function (query) {
		    	$.ajax({
				   type: "POST",
				   url: "delivery.do?method=selectChangeCar",
		...
		$("#changecompany").select2({
		    placeholder: "请输入中转公司",	//默认文本框显示信息
		    minimumInputLength: 1,	//最小输入字符长度
		    allowClear:true,
		    multiple: false,				//设置多项选择，false则只能单选
		    maximumSelectionSize: 5, //最大选择个数
			query: function (query) {
		    	$.ajax({
				   type: "POST",
				   url: "delivery.do?method=selectCompany",
				   data: {company:query.term},   //传递你输入框中的值
				   success: function(msg){
		 		   	var msg=$.parseJSON(msg);		
		 			var data = {results: []};		
					$.each(msg,function(x,y){
  	...
  	/* 	 if($.trim($("#phone").select2("data"))!=""){
  		...
  		$("#dg").datagrid('load',{
  				...
  				changecompany:$.trim($("#changecompany").select2("val")),
  	...
  	function putoutfile(){
		...
		for(i=1;i<allRows2.length;i++)		
		{
			var col = $('#dg').datagrid( "getColumnOption" , allRows2[i] );
		 	if(col.field!="send_type"&&col.field!="shipping_order_state"&&col.field!="pay_type"&&col.field!="write_off"&&col.field!="chanage_pay_type"){ 
  ...
  <body class="easyui-layout">
  	<form action="delivery.do" id="outputform" method="post">
    	<div region="north" title="检索" class="st-north" border="false" collapsible="false"  >
    	 <lable>中转日期:</lable>
			<input  id="send_time" name="send_time" class="Wdate"  readonly="readonly" style="width:130px;height:22px; " onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
			-
			<input id="end_date" name="end_date" class="Wdate"  readonly="readonly" style="width:130px;height:22px;" onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
	 	
	    	<lable >订单编号:</lable>
	    	 <input type="text" class="search-text" id="num" name="num" style="width:130px;margin-right:0px"  >
	    	 
	    	<lable >中转公司:</lable>
	    	 <input type="hidden" class="search-text" id="changecompany" name="changecompany" style="width:130px;margin-right:0px"  >    	 
	    	    <lable >收货人:</lable>
	    	 <input type="text" class="search-text" id="receipt"  style="width:130px"  >	    	 	    		 	 
	    	 ...
	    	 <lable >品名:</lable>
	    	 <input type="text" class="search-text" id="goods_name" name="goods_name" style="width:130px;margin-right:0px"  >     
	    	 <lable >件数:</lable>
	    	 <input type="text" class="search-text" id="goods_num" name="goods_num" style="width:130px;margin-right:0px"  > 	
			...
			<lable>到&nbsp;站:</lable>
			<input type="text" class="search-text" id="endaddress"name="endaddress"style="width:130px;margin-right:0px" >
			  <lable>车牌号:</lable>
	    	 <input  type="hidden" class="search-text" id="carnum" name="carnum"  style="width:130px"  >	
			</div>
    	</div>
    	<div region="center" >
    		<table id="dg" title="转运信息" ></table>
    	</div>
    	<div id="tb">
		...
		<input type="hidden" name="method" value="outgettransferGoods">
		<input type="hidden" id="fieldName" name="fieldName">
		<input type="hidden" id="headtitle" name="headtitle">
		<input type="hidden" id="did" name="customid">
		</form>
		 <form action="dlivery.do" target="_blank" id="printywm" method="post">
