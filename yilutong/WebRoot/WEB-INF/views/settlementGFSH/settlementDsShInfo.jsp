  ...
  <script type="text/javascript">
 		...
 		var grid;
  		...
  		$(function(){
			...
			$("#flag").combobox("setValue", "4");
  			...
  			grid=$("#dg").datagrid({
  			url : 'settleDaiskgf.do?method=getSettlementYfSHInfo&isdelivery='+did,
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		rowStyler: function(index,row){
  			...
  			columns : [[
			  	...
			  	{field : 'settlgf_sh',title : '审核状态', width : 80,align :'center',sortable:'true',
  				...
  				{field : 'trade_agency',title : '应收代收款总额',width : 100,align:'center'/* sortable: true */},
  				{field : 'paidgf_fu',title : '实收代收款总额',width : 100,align:'center'	/* sortable: true */},
  				{field : 'trade_cha',title : '差异',width : 70,align:'center',
  				formatter:function(val,row,index){			
		  					if(isNaN(parseFloat(row.settlgf_fu))){
					  		 val=row.trade_agency;
					  		}else{
					  		 val=parseFloat(row.trade_agency)-parseFloat(row.settlgf_fu); 	
					  		}	  		
  							return val;		
  					}
  			
  				},  	
  				{field : 'is_settlgf',title : '是否结算', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  							if(val==0){
  							return "未结算";
  						}else if(val==1){ 						
		  	...
		  	if(ids.length>0){
				...
				buttons : [{
				...
				},
				 {
  				...
  				$.messager.alert("代收款结算","请选择已提审信息","info");
  		}	
  	}
  	//修改
  	function updateUser(){
	...
	var ids=[];
  		var row=$("#dg").datagrid('getSelections');
		  	...
		  	if(ids.length>0){
	  			dialog = parent.jy.modalDialog({
				title : '结算审核',
				url : 'settleDaiskgf.do?method=shSettlement&pid=' + ids.join(","),
				width : 600,
				height:420,
				buttons : [{
					text : '<input type="button" class="btncss" value="通过"/>',
	...
	//查看审核详情
  		function OrderUpdates(ids){
  			dialog = parent.jy.modalDialog({
			title : '代收款结算审核记录',
			url : 'settleDaiskgf.do?method=settlementShRC&pid='+ids,
/* 				 buttons :'#addbuttons'
*/
			width : 690,
			height:500,
