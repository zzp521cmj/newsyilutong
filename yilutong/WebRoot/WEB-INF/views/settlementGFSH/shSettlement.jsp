  ...
  var pids=[];
 	pids.push(pid);
  	$(function(){
  		...
  		$.ajax({
  		  	url: 'settleDaiskgf.do?method=getSettlout',
  		  	data:{id:pids.join(",")},
  		  	success:function(data){
  		  		data.trade_agency=parseFloat(data.trade_agency)-parseFloat(data.settlgf_fu.split(",")[1]);
  		  		data.paidgf_fu=parseFloat(data.settlgf_fu.split(",")[0])-parseFloat(data.settlgf_fu.split(",")[1]); 
 	...
 	var submitFormEdit= function($dialog, $grid, $pjq, $mainMenu) {	
 	 ...
 	 //可提交
		$pjq.messager.confirm('代收款结算','确定要提交吗?',function(r){   
	    	if (r){    
				$.ajax({
					type: "POST",
		  			url:'settleDaiskgf.do?method=settlementYfTG',
		  			data:{
		  			ids:pids.join(","),
		  			notes:$("#remarks").val()
		  			},/* $('#Editform').serialize() */
		  			success:function(msg){
		  				if(msg.flag){
		  					$pjq.messager.alert('结算审核','运单代收款结算审核通过成功','info');
 	 ...
 	 //可提交
		$pjq.messager.confirm('代收款结算','确定要提交吗?',function(r){   
	    	if (r){    
				$.ajax({
					type: "POST",
		  			url:'settleDaiskgf.do?method=settlementYfBH',
		  			data:{
		  			ids:pids.join(","),
		  			notes:$("#remarks").val()
		  			},/* $('#Editform').serialize() */
		  			success:function(msg){
		  				if(msg.flag){
		  					$pjq.messager.alert('结算审核','运单代收款结算审核驳回成功','info');
						   	$grid.datagrid('reload');
						   	$dialog.dialog('close');
		  				};
		  			}
  	...
  	<table class="tableclass">
		...
		<tr>
	 	<td><font color="red" style="margin-right:10px">*</font>运单号:</td>
			...
			<td><font color="red" style="margin-right:10px">*</font>应收代收款:</td>
			<td class="td2">
				<input type="text" class="validate[required]" 	readonly="true" id="price" name="trade_agency" />
	      	</td>
	    </tr>
	    <tr>			
			<td><font color="red" style="margin-right:10px">*</font>实收代收款:</td>
			<td class="td2">
				<input type="text" class="validate[required]" readonly="true" id="price1" name="settlgf_fu" />
	      	</td>
	      	<td>			
