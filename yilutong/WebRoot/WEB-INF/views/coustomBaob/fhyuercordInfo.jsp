...
<html style="height: 100%">
		...
		function add(num,nums){
      	...
      	$(function(){
			...
			query: function (query) {
      		...
      		<%				     		
			String data2 = new SimpleDateFormat("yyyy-MM").format( Calendar.getInstance().getTime());				
				%>
      		$("#endDate").val('<%=data2%>');  		
 	     		searcher();
      		});
   function searcher(){
  			 ...
  			 $.ajax({
				   type: "POST",
				   url: "coustomB.do?method=getOrderInfoM",
				   data: 
				   {
				   end:$("#endDate").val(),
				   ...
				   success: function(msg){
				 		 	if(msg.ans=="err"){
				 		 	$.messager.alert("月发货图","没有对应的数据值","info");
				 		 	}else{
        ...
        <div region="north" class="st-north" border="false"  height="28px" collapsible="false"  >
			...
			<input id="endDate"  name="end_date"  class="Wdate"  readonly="readonly" style="width:120px;height:21px;" onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM'})" />
