...
<html>
  ...
  var pid=[ ];
 	...
 	var submitFormEdit = function($dialog, $grid, $pjq, $mainMenu) {
 	 ...
 	 //可提交
		$pjq.messager.confirm('异常上报','确定要提交吗?',function(r){   
	    	if (r){    
				$.ajax({
					type: "POST",
		  			url:'unqualified.do?method=updatedocuments',
		  			data:{
		  			pid:pid.join(","),
		  			...
		  			success:function(msg){
		  				if(msg.flag){
		  					$pjq.messager.alert('上报提交','信息提交成功','info');
			    ...
			    $pjq.messager.alert('异常上报', '请选择提审部门', 'info');
			  }
	};
