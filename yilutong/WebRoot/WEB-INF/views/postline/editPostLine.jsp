...
<% 
	...
	if(request.getAttribute("pid")!=null){
		flg=request.getAttribute("pid").toString();
	}
 %>
...
var linenum=0;//线路个数
$(function(){
$("#editform,#form_F,#form_E,#form_P").validationEngine('attach', {
	...
	if(pid!=null){
		$.ajax({
  		  	...
  		  	success:function(data){
  		  		...
  		  		//放到加载load数据之前选中check
  		  		$("#endtype_"+data.postClassline.end_type).iCheck('check');
  		  		if(data.postClassline.regular_type==0){
					$("#datetype_2").iCheck('check');
 					$("#weeklable").show();
					$(".repetitionClass").show();
					$("#selectId").hide();
					$("#daysId").hide();
					var classweek_arr = data.postClassline.repetition_period.split(',');
					$.each(classweek_arr,function(i,itme){
						/* var num = parseInt(i)+1;
						alert(); */
						$('#classweek_'+itme).iCheck('check');
						
					});
				}else{
					$("#datetype_1").iCheck('check'); 
 					$("#selectId").val(data.postClassline.days);
				}
  		  		$('#editform').form('load', data.postClassline);
  		  		var cars = data.postClassline.plate_number ? data.postClassline.plate_number.split(',') : ''; 
  		  		var truckids=data.postClassline.car_id ? data.postClassline.car_id.split(',') : ''; 
  		  		if(cars.length == 1){
					$("#truck_id").select2("data", {id:data.postClassline.car_id, text:data.postClassline.plate_number});
				}else if(cars.length > 1){
					var morecar = [],moretruckids = [];
					$.each(cars, function(k,v){
 						morecar.push({id:truckids[k], text:v});
 					});
 					$('#truck_id').select2('data', morecar);
					$('#truckid').val(data.truckids);
 				}
  		  		//机构
				$('#cc').combotree('setValue',data.postClassline.organizationName);
				//发车时间
				$("#departure_timeId").val(data.postClassline.departure_time.substring(0,16));
  		  		...
  		  		$.each(data.linesinfo,function(i,item){
					if(item.isdefalut == 1){
						var Pass = {};
						Pass.line_name = item.linname;
						Pass.line_id = item.linid;
						Pass.line_mlg = item.lincd;
						Line_default = 0;
						Pass.line_default = item.isdefalut;
 ...
 function show_line(){
	...
	$.each(Lines, function(i, item) {
		if(item.line_id){
			...
			+"<th>"+line_mlg+"</th>"
			//<a class='btn btn-default btn-xs' onclick="+"modal_Line(true,'"+item.linenum+"')"+">修改</a>
...
var submiteditForm = function($dialog, $grid, $pjq, $mainMenu) {
		//获取select2选择的车辆
		...
		if($("#editform").validationEngine('validate')){
			$pjq.messager.confirm('修改班线','确定要修改班线吗?',function(r){   
		    	if (r){
		    	var pararm = $("#editform").serializeArray();
				...
				pararm.push({'name':'lines','value':lin});
					 $.ajax({
						type: "POST",
			  			url:'postline.do?method=editPostLineInfo',
			  			data:pararm,
			  			success:function(msg){
			  				 if(msg.flag){
			  					$pjq.messager.alert('修改班线','新增修改成功','info');
...
<table  class="tableclass">
	...
	<tr>
		...
		<td class="td2">
			<input type="hidden" id="classlineId" name="classlineId"/>
		...
		<td class="td2">
			<input id="cc" name="organizationId" style="width: 158px;">
		</td>
	</tr>
	<tr>
	...
	<table class="tableclass" style="margin-top: 10px;">
			...
			<tr>
				...
				<td class="td2">
					<input type="radio" id="datetype_1" value="1" name="regular_type">
				...
				<td class="td2">
					...
					<label class="repetitionClass">
						<input type="checkbox" id="classweek_1" name="repetition_periods"  value="1">
						每逢 周一 发车
					</label>
					<label  class="repetitionClass">
						<input type="checkbox" id="classweek_2" name="repetition_periods"  value="2">
						每逢 周二 发车
					</label>
					<label  class="repetitionClass">
						<input type="checkbox" id="classweek_3" name="repetition_periods"  value="3">
						每逢 周三 发车
					</label>
					<label  class="repetitionClass">
						<input type="checkbox" id="classweek_4" name="repetition_periods"  value="4">
						每逢 周四 发车
					</label>
					<label  class="repetitionClass">
						<input type="checkbox" id="classweek_5" name="repetition_periods"  value="5">
						每逢 周五 发车
					</label>
					<label  class="repetitionClass">
						<input type="checkbox" id="classweek_6" name="repetition_periods"  value="6">
						每逢 周六 发车
					</label>
					<label  class="repetitionClass">
						<input type="checkbox" id="classweek_7" name="repetition_periods"  value="0">
						每逢 周日 发车
					</label>
				</td>
			</tr>
			<tr>
				...
				<td class="td2" colspan="3">
					<input type="radio" id="endtype_1" value="1" name="end_type" >
					...
					计划发生次数:
					<input type="text"  name="end_num"  placeholder="365" value="365" class="validate[required]">
					</label>
	...
	<div id="Dialog_P">
		...
		<table class="tableclass">
			...
			<tr>
				...
				<td class="td2" >
					<input type="text" class="validate[required,custom[number],min[0],max[999]]" style="width: 88px;" value="0" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" onchange="this.value = this.value=='' ? 0: this.value" id="Stoptime_P_T" name="Stoptime_P_T">
					<label>时</label>
					<input type="text" class="validate[required,custom[number],min[0],max[999]]" style="width: 58px;" value="0" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" onchange="this.value = this.value=='' ? 0: this.value" id="Stoptime_P_M" name="Stoptime_P_M">
					<label>分</label>
				</td>
			</tr>
		</table>
