...
<html>
  ...
  <script type="text/javascript">
	...
	$("#tbput").removeAttr("style");
  			...
  			grid=$("#dg").datagrid({
  			url : 'jmonitor.do?method=getJITMonitor&monitorType=1',
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
  			...
  			columns : [[
  				...
  				{field : 'time_real',title : '实际发车时间',width : 140,align:'center'},
  				{field : 'time_plan',title : '计划发车时间',width : 140,align:'center'},
  	...
  	function searchmonitorRuning(){
		...
		// 没有格式化的功能，只能一个一个取 
  		...
  		$("#dg").datagrid('load',{
	 	 		...
	 	 		end_date :endDate,
	 	 		//monitorType:$("#type").combobox("getValue"),
	 	 		monitorType:1,
    				...
    				$.ajax({
						...
						data:{del:del},
			  				success:function(data){
			  					if (data.flag) {
									parent.$.messager.alert("发运延迟监控","发运延迟监控处理成功",'info',
										function(r) {
											$('#dg').datagrid('reload');
 										});
  								} 
  	...
  	<form action="jmonitor.do" id="outputform" method="post">
    	...
    	<div id="tb">
    		<a href="javascript:void(0)" class="easyui-linkbutton" style="display: none" id="tbchuli" data-options="iconCls:'icon-edit',plain:true" onclick="dealJITmonitor()">处理</a>
		  	<a href="javascript:void(0)" class="easyui-linkbutton" id="tbput" style="display: none" data-options="iconCls:'icon-output',plain:true" onclick="putoutfile()">导出</a>
		</div>
		
	<input type="hidden" name="method" value="outJITMonitor">
