...
<html>
  ...
  <script type="text/javascript">
		...
		<%for(int i=0;i<flg.length;i++){
           ...
           if (fee == "null") {
			$("#destination_fee").val(0);
		} else {
           $("#destination_fee").val(fee);
		}
  ...
  var submitFormEdit = function($dialog, $grid, $pjq, $mainMenu) {
		...
		$pjq.messager.confirm('费用结算', '确定要结算吗?', function(r) {
		    ...
		    var paids = $('#paid_fee').val();
		    if(paids == null || paids == ""){
		    paids = "0.0";
		    }
		    
		    var paid_fee =  parseFloat(paid)+parseFloat(paids);
/* 		    var radionum = document.getElementById("Regform").write_off;
			...
			if (r) {
				$.ajax({
					type : "POST",
					url : 'Destination.do?method=updateCollect&pid='+pid.join(","),
					data : {
					paid_fee:paid_fee,
    ...
    <input type="hidden" id="customid"  name="custom_id"  >
 	<table class="tableclass">
 	    <tr>
		    <td >应收代收运费:</td>  
			<td class="td2">
				<input  id="destination_fee"	style="width:120px;" name="destination_fee" disabled="disabled">
			</td>
			</tr>
			<tr>	
			<td>实收代收运费:</td>
			<td class="td2">
			    <input  id="paid_fee" style="width:120px;" name="paid_fee" class="validate[custom[number],max[99999.99],min[0.00],custom[dd],required]">
			</td>			
		</tr>
<!-- 			<tr>
					<td>			
