  ...
  <script type="text/javascript">
 		...
 		var editval;
  		...
  		$('#dlg1').dialog({
			...
			$("#kname").val('<%=flg%>');
  			grid=$("#dg").datagrid({
  			url : 'settleTC_fu.do?method=getSettlementYf&kname='+'<%=flg%>',
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		rowStyler: function(index,row){
				if (row.settltc_zt!=2||row.is_settltc==0||row.settltc_sh==3||(row.settltc_sh==2&&(parseFloat(row.change_pay)!=parseFloat(row.paidtc_fu)))){
					return 'color:blue;';
				}else if (row.settltc_sh==0){
					return 'color:rgb(221, 14, 211);';
				}else if(row.settltc_sh==1){
					return 'color:red;';
				}
			},
  			columns : [[
  				...
  				//{field : 'trade_agency',title : '应付代收款总额',width : 100,align:'center'/* sortable: true */},
  				...
  				{field : 'settltc_zt',title : '结算调整', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  							 if(val==1){ 						
  						return "调整未结算";
  						}else if(val==2){ 						
  					...
  					{field : 'settltc_sh',title : '是否审核结算', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  						if(val==0||val==null){
  							return "未审核";
  						}else if(val==1){ 						
  	...
  	function searchCar_owner(){
  		...
  		$("#dg").datagrid('load',{
	 	 	...
	 	 	//	phone_number:$.trim($("#phone").val()),
  				...
  				isdelivery:$.trim($("#isdelivery").combotree("getValue")),//是否提货
  		...
  		var row=$("#dg").datagrid('getSelections');
  			if(row.length>0){
	  			for(var i=0;i<row.length;i++){
	  				if(row[i].settltc_sh==0&&row[i].settltc_zt==2){
		  	...
		  	if(ids.length>0){
				...
				title : '结算提审',
				url : 'settleTC_fu.do?method=thSettlement&pid=' + ids.join(","),
				width :450,
				height:300,
				buttons : [{
  				...
  				$.messager.alert("结算调整","请选择已结算未审核信息","info");
  		}		
  	}
  	//修改
  	function updateUser(){
		  ...
		  var wids=[];
		 	var row=$("#dg").datagrid('getSelections');
	  		...
	  		//&&(parseFloat(row[i].transport_pay)!=parseFloat(row[i].reality_sum)
		  			if(row[i].is_settltc==0||(row[i].is_settltc!=0&&row[i].settltc_sh==3&&row[i].settltc_zt!=2)||(row[i].settltc_zt!=2&&row[i].is_settltc!=0)){
			  				ids.push(row[i].shiping_order_id);	  				
		  				}		  			
		  			}	
		  if(ids.length>0){
				  dialog= parent.jy.modalDialog({
					title : '中转费结算',
					url : 'settleTC_fu.do?method=edSettlement&pid='+ids.join(","),
		/* 				 buttons :'#addbuttons'
		*/
					width : 530,
					height: 270,
		  	...
		  	$.messager.alert("运单结算","请选择未结算或者已调整未结算的信息","info");
		  }
	
		}else {
			$.messager.alert("运单结算","请选择未结算或者已调整未结算信息","info");
		}
	};
  	//打印
  	function dy(){
	...
	//调整金额
		function changemoney(){
			 var ids="";
				var row=$("#dg").datagrid('getSelections');
		  			if(row.length==1){
			  			for(var i=0;i<row.length;i++){		  			
			  			if(row[i].is_settltc==0||(row[i].settltc_zt==1)||(row[i].settltc_zt!=1&&(row[i].settltc_sh!=0&&row[i].settltc_sh!=1)&&(parseFloat(row[i].change_pay)!=parseFloat(row[i].paidtc_fu)))){
			  				ids=row[i].shiping_order_id;	  				  								
			  				}	  		
				  		}
					if(ids!=""){
						$("#form").form('clear');			
						if(row[0].is_settltc==0&&row[0].trade_cha==0){
			  					$("#yingshou").val(row[0].change_pay);
			  				}else if(isNaN(parseFloat(row[0].paidtc_fu))){
			  					$("#yingshou").val(row[0].change_pay);	
			  				}else{
			  					$("#yingshou").val(parseFloat(row[0].change_pay)-parseFloat(row[0].paidtc_fu));
			  					}	  									
						$("#dlg1").dialog('open');
						}else{
		...
		function dealOrder(){
	  		var row=$("#dg").datagrid('getSelections');
	  		var reality_sum;
	  		if(isNaN(parseFloat(row[0].paidtc_fu))||(row[0].is_settltc==0&&row[0].settltc_zt==0)){
	  		 reality_sum=parseFloat($("#shishou").val());
	  		}else{
	  		 reality_sum=parseFloat($("#shishou").val())+parseFloat(row[0].paidtc_fu);
	  		}	  		
				  			$.messager.confirm('确认','您确定要提交处理吗？',function(r){ 
			    			if (r){ 
			    				$.ajax({									
									type: "POST",
									url : 'settleTC_fu.do?method=changePrice',
				           			data :  {
  		...
  		function OrderUpdates(ids){
  			dialog = parent.jy.modalDialog({
			title : '中转费结算记录',
			url : 'settleTC_fu.do?method=setlRcordInfo&pid='+ids+'&state='+1,
/* 				 buttons :'#addbuttons'
*/
			width : 690,
			height:500,
