...
<html>
  ...
  <script type="text/javascript">
	...
	$("#tbput").removeAttr("style");
  			...
  			grid=$("#dg").datagrid({
  			url : 'monitor.do?method=getMonitorRuning&monitorType=2',
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
  			...
  			columns : [[
  				...
  				{field : 'monitor_plan_range',title : '计划行驶距离',width : 95,align:'center'},
  				{field : 'monitor_real_range',title : '实际行驶距离',width : 95,align:'center'},
  				{field : 'out_of_range',title : '超出距离',width : 70,align:'center'},
  				{field : 'monitor_state',title : '状态',width : 70,align:'center',
  					formatter:function(val,row,index){
  						if(val==0){
  							return "未解决";
  						}else if(val==1){
    				...
    				$.ajax({
						...
						data:{del:del},
			  				success:function(data){
			  					if (data.flag) {
									parent.$.messager.alert("距离监控","距离监控处理成功",'info',
										function(r) {
											$('#dg').datagrid('reload');
 										});
  								} 
  ...
  <body class="easyui-layout">
  	<form action="monitor.do" id="outputform" method="post">
    	...
    	<div id="tb">
    	 		<a href="javascript:void(0)" class="easyui-linkbutton" style="display: none" id="tbchuli" data-options="iconCls:'icon-edit',plain:true" onclick="dealmonitorRuning()">处理</a>
	...
	<input type="hidden" name="method" value="outMonitor2">
