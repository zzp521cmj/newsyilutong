...
<html>
  ...
  <script type="text/javascript">  
  	var pid='<%=pid%>';
  		$(function(){
		 $("#dg").datagrid({
			url : 'transpfree.do?method=getTranfreeShRc&ddids='+pid,
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
  			...
  			columns : [[
  				...
  				{field : 'checks_times',title : '审核操作时间',width : 140,align:'center',
  				...
  				{field : 'checks_type',title : '审核类型',width : 90,align:'center',
  					formatter: function(value,row,index){
		  				if(value==1){
		  				return '审核操作';
		  				}else if(value==0){
  				...
  				{field : 'checks_state',title : '审核状态',width : 90,align:'center',
  					formatter: function(value,row,index){
		  				if(value==1){
		  				return '审核通过';
		  				}else if(value==0){
  				...
  				{field : 'checks_notes',title : '备注信息',width : 140,align:'center'},				
  			]],
  			pagination:true,//分页
		    pageSize:10,
   ...
   <form action="car_b_rec.do" id="outputform" method="post">
  	<div region="north" title="检索" class="st-north" border="false" collapsible="false"  >
   	<lable >审核日期:</lable>
    	...
    	<div region="center" >
    		<table id="dg" title="运费调整审核记录信息"></table>
    	</div>
    	<div id="tb">
