...
<script type="text/javascript">
        ...
        grid=$("#dg").datagrid({
            ...
            columns : [[
              ...
              /*   {field : 'pay_type',title : '付款方式',align:'center',
                ...
                {field : 'all_transportation_fee',title : '运单总运费用',align:'center',
                },
                {field : 'all_trade_agency',title : '运单总代收款',align:'center',
                },
                {field : 'all_destination_fee',title : '运单总代收运费',align:'center',
                },
    ...
    function xiangxixinxi(){
        ...
        if(row.length==1){
            ...
            var payment_sh=row[0].payment_sh;
            var all_money=row[0].all_money;
            var receivable_fee=row[0].receivable_fee;
            if(payment_sh==2&&all_money==receivable_fee){
    ...
    //协议推送
   /*  function pushCAMessage(){
        var agreement_id=[];
        //var check=[];
        var array = $('#dg').datagrid('getSelections');
        for(var i=0;i<array.length;i++){
           if(array[i].car_name==''||array[i].car_name==null){ 
              agreement_id.push(array[i].agreement_id);
           }
        }
        if(agreement_id != ""){
            $.messager.confirm('确认','您确定推送选中的记录吗？',function(r){
                if (r){
                    $.ajax({
                        url: "createA.do?method=pushCAMessage",
                        type: "POST",
                        data:{agreement_id:agreement_id.join(","),
                        },
                        success:function(msg){
                           var zs=msg.split("#")[0];
					       var cg=msg.split("#")[1];
                           var ms="<span>推送成功<br>推送司机数量为 ："+zs+"<br>推送协议数量为："+cg+"</span>";
                           $.messager.alert('推送成功', ms, 'info');
            ...
            $.messager.alert("协议推送","请选择要推送的协议，或者选择的协议已经确定车辆","info");
        }
    } */
    //打印承运协议
    function pushCAMessage(){
        var row=$("#dg").datagrid('getSelections');
        if(row.length==1){
            var id=row[0].agreement_id;
            var url = "createA.do?method=printerCAMessage&agreementId=" + id;
			ow(url);
            /*  dialog = parent.jy.modalDialog({
                title : '协议打印',
                url : 'createA.do?method=printerCAMessage&agreementId=' + id,
                width : 1100,
                height : 600,
                onDestroy:function(){
                	$('#dg').datagrid('reload');
 ...
 function ow(owurl) {
		var tmp = window.open("about:blank", "", "fullscreen=1");
		tmp.moveTo((screen.width-800)/2, 0);
		tmp.resizeTo(800, screen.height);
		tmp.focus();
		tmp.location = owurl;
	}
</script>
<body class="easyui-layout">
	...
	<!-- 	<div id="tb">
			<a href="javascript:void(0)" class="easyui-linkbutton"
				data-options="iconCls:'icon-add',plain:true" onclick="createAgreement()">制作协议</a>
			<a href="javascript:void(0)" class="easyui-linkbutton" id="modify"
				data-options="iconCls:'icon-edit',plain:true" onclick="xiangxixinxi()">详细信息</a>
			<a href="javascript:void(0)" class="easyui-linkbutton"
				data-options="iconCls:'icon-remove',plain:true"
				onclick="deleteAgreement()">作废</a>
			<a href="javascript:void(0)"	class="easyui-linkbutton" data-options="iconCls:'icon-ok',plain:true" onclick="pushCAMessage()">推送协议</a>
