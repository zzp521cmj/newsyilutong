...
<html>
  ...
  <script type="text/javascript">
 		...
 		var editval;
  		$(function(){
  			grid=$("#dg").datagrid({
  			url : 'receiptB.do?method=getReceiptBaob',
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		/* 	remoteSort:true, */
  			columns : [[
  				...
  				{field : 'sign_time',title : '提货签收日期',width : 130,align:'center',
  			...
  			/* 	{field : 'car_number',title : '车牌号',width : 70,align:'center'}, */
  				...
  				{field : 'sign_usernumber',title : '提货人证件',width : 90,align:'center'},
  				{field : 'sign_name',title : '提货代理人',width : 70,align:'center'},
  				{field : 'sign_number',title : '提货代理人证件',width : 90,align:'center'},  				
 	...
 	//周图
  	function day(){
  		var row=$("#dg").datagrid('getSelections'); 				
				...
				title : '收货量日图',
				url : 'receiptB.do?method=ddrcordInfo' ,
				width : 680,
				height:500,
				buttons : [{
					handler : function() {
  	...
  	function zhou(){
  		var row=$("#dg").datagrid('getSelections'); 				
	  			dialog = parent.jy.modalDialog({
				title : '收货量周图',
				url : 'receiptB.do?method=dayrcordInfo' ,
				width : 680,
				height:500,
				buttons : [{
					handler : function() {
  	...
  	function yue(){
  		var row=$("#dg").datagrid('getSelections'); 				
	  			dialog = parent.jy.modalDialog({
				title : '收货量月图',
				url : 'receiptB.do?method=yuercordInfo' ,
				width : 680,
				buttons : [{
					handler : function() {
						dialog.find('iframe').get(0).contentWindow.submitFormEdit(dialog, grid, parent.$);
  	...
  	function nian(){
  		var row=$("#dg").datagrid('getSelections'); 				
	  			dialog = parent.jy.modalDialog({
				title : '收货量年图',
				url : 'receiptB.do?method=nianrcordInfo' ,
				width : 680,
				buttons : [{
					handler : function() {
						dialog.find('iframe').get(0).contentWindow.submitFormEdit(dialog, grid, parent.$);
  ...
  <body class="easyui-layout">
  	<form action="receiptB.do" id="outputform" method="post">
	 ...
	 -->	</div>	
		<input type="hidden" name="method" value="outReceiptBaob">
