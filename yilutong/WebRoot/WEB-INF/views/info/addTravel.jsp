...
<script type="text/javascript"
	src="./webuploader/travel_uplaod.js"></script>
...
<link rel="stylesheet" type="text/css" href="./webuploader/style.css" />
  ...
  <script type="text/javascript">
  $(function(){
   $("#addTravel").validationEngine('attach', { 
...
var xiangxing=$("#box_add").combobox('getText'); 
	if(bumen==null||bumen==""){
		$pjq.messager.alert('新增车辆', '请选择所属部门', 'info');
	}else{
		
		if(xiangxing==null||xiangxing==""){
		
		$pjq.messager.alert('新增车辆', '请选择厢型', 'info');
		}else{
		  	var flag;
			  	$.ajax({
							type : "POST",
			  	 ...
			  	 if($("#addTravel").validationEngine('validate')&&flag!=true){
					...
					$pjq.messager.confirm('新增车辆','确定要新增吗?',function(r){
				    	if (r){ 
							...
							$.ajax({
								type: "POST",
					  			url:'travel.do?method=addCar',
					  			data:$('#addTravel').serialize(),
  ...
  <body>
    <form action="" method="post" id="addTravel">
    	<fieldset>
    	<table class="tableclass">
    		...
    		<tr>
	    		...
	    		<td class="td2"><input type="text" class="validate[custom[phone],required]" name="tels"/></td>
	    		<td><font color="red" style="margin-right:10px">*</font>备用电话</td>
	    		<td class="td2"><input type="text" class="validate[custom[phone],required]" name="telstoo"/></td>
	    		</tr>
    		<tr>
    			...
    			<td><font color="red" style="margin-right:10px">*</font>厢型</td>
   				<td class="td2"><input class="easyui-combobox" id="box_add" class="validate[required]" name="boxType"/></td>
	    		</tr>
    			<tr>
    	...
    	<table class="tableclass">
    		...
    		<tr>
	    		...
	    		<td>吨（座）位</td>
	    		<td class="td2"><input type="text" class="search-text" name="tonnage"/></td>
    		</tr>
    		<tr>
	    		<td>车宽</td>
    	...
    	<table class="tableclass">
    		...
    		<tr>
	    		<td>维护日期</td>
	    		<td class="td2"><input id="addDate"type="text" name="maintenanceDate" class="Wdate" onfocus="WdatePicker({skin:'twoer',dchanging:cDayFunc})" style="width: 150px;"/></td>
	    		<td>下次维护日期</td>
