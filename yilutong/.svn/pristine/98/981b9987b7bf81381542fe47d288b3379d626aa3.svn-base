  ...
  <script type="text/javascript">
	...
	btnMoreSearch.click(function() {
		 ...
		 //主要代码 
  		...
  		grid=$("#dg").datagrid({
  			...
  			columns : [[
  				...
  				{field : 'start',title : '起始地',width : 110,align:'center'},
  				{field : 'end',title : '到达地',width : 110,align:'center'},
  	...
  	function searchDemandRelease(){
		$('#dg').datagrid('load',{
			start : $.trim(document.getElementById("start").value),
			end : $.trim(document.getElementById("end").value),
	...
	function xiangxixinxi() {
		...
		if(row.length==1){
			...
			dialog = parent.jy.modalDialog({
				title : '详细信息',
				url : 'temvehicle.do?method=getxinxiDemandRelease&pid=' + pid,
				width : 800,
				height:480,
				/* buttons : [{
	...
	<div region="north" title="检索条件" class="st-north" border="false" data-options="collapsible:false" style="height:62px">
		<label>起始地：<input type="text" id="start" /></label>
		<label class="divlabel" >到达地：<input type="text" id="end" /></label>
