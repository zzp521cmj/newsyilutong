...
<script type="text/javascript">
	...
	$(function() {
		...
		$.ajax({
			type : "POST",
			url : 'unqualified.do?method=getselectunqualified&pid='+pid,
	...
	//var pid = document.getElementById("flg").value？那你动啊
	    		...
	    		if (r){    
					$.ajax({
					type: "POST",
		  			url:"unqualified.do?method=getUpdateunqualifie",
		  			data:$('#Regform').serialize(),
		  			success:function(msg){
		  				if(msg.flag){
		  					$pjq.messager.alert('修改信息','信息编辑成功','info'); 
									   $dialog.dialog('close');
	...
	<form action="" method="post" id="Regform">
		...
		<fieldset>
			<table class="tableclass">
				...
				<tr>
					<td>运单号:</td>
					<td class="td2" colspan="3"><input readonly="true" type="text"
						id="shiping_order_num" class="validate[required]" style="width:200px" name="shiping_order_num" ><font color="red"
						style="margin-right:10px">运单信息不可修改</font> <!-- <a
				...
				<tr>
					...
					<!-- <select class="easyui-combobox" id="types_id" name="types_id"
					style="width:120px;"
					size="2">
					...
					<td>货品名称：<font color="red" style="margin-right:10px">*</font></td>
					<td class="td2"><input type="text" readonly="true"
						id="goods_name" name="goods_name">
					</td>
				</tr>
				<tr>
					<td>不合格件数：<font color="red" style="margin-right:10px">*</font></td>
					<td class="td2"><input type="text" class="validate[required[custom[number]]]"
						name="unqualified_number">
					</td>
					<td>代收款：
					</td>
					<td class="td2"><input type="text" readonly="true"
						id="trade_agency" name="trade_agency" /></td>
				</tr>
				<tr>
					<td>负责机构：<font color="red"
						style="margin-right:10px">*</font>
					</td>
					<td class="td2"><input type="text" class="validate[required]"  name="responsible_institutions" id="responsible_institutions" />
					</td>
					<td>负责人：<font color="red"
						style="margin-right:10px">*</font>
					</td>
					<td class="td2"><input type="text" class="validate[required]" name="responsible_people" id="responsible_people" /></td>
				</tr>

				<tr>
					<td>参考价格：</font>
					</td>
					<td class="td2"><input type="text" value="0" class="validate[custom[number]]" name="reference_price" id="reference_price" />
					</td>
					<td>上报人姓名：<font color="red"
						style="margin-right:10px">*</font>
					</td>
					<td class="td2"><input type="text" value="盖仓操作员"
						name="report_people" id="report_people" />
					</td>
				</tr>
				<tr>
					<td>协议编号：
					...
					<td>协议实收数量：</td>
					<td class="td2"><input id='agreement_paid' name="agreement_paid" class="validate[required[custom[number]]]" type="text"> <input
						type="hidden" readonly="true" name="check_type"></td>
				</tr>
				<tr>
				...
				<tr>
					<td>异常情况：</td>
					<td class="td2" colspan="3"><input type="text"   id="abnormal_situation" name="abnormal_situation"
						style="width:350px">
						<input type="hidden" name="customer_id" id="customer_id" /></td>
