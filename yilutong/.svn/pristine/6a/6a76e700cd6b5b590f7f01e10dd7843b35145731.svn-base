...
//通过按钮
     var submitFormtongguo=function($dialog, $grid, $pjq, $mainMenu){
       // var audit=document.getElementById("audit_state").value;
              // if(audit==2){
      ...
      $pjq.messager.confirm('审核', '确定要审核吗?', function(r) {
				...
				if (r) {
					$.ajax({
						type : "POST",
						url : 'inf.do?method=update&abnormalid='+abnormalid,
   	  ...
   	  // var audit=document.getElementById("audit_state").value;
			...
			$pjq.messager.confirm('驳回', '确定要驳回吗?', function(r) {
				if (r) {
					$.ajax({
						type : "POST",
						url : 'inf.do?method=bohui&abnormalid='+abnormalid,
						data : $('#editTravel').serialize(),
						success : function(msg) {
							if (msg.flag) {
...
<body>
	<form action="" method="post" id="editTravel">
		<fieldset>
			...
			<textarea class="easyui-textarea" class="validate[maxSize[64]]" cols="40" rows="2" id="reason" name="reason"></textarea>
		</td>
				</tr>
