...
<html>
  ...
  var submitFormEditss=function($dialog, $grid, $pjq, $mainMenu) {
		...
		$pjq.messager.confirm('配送费结算', '确定要结算提交吗?', function(r) {
			if (r) {
				$.ajax({
					type : "POST",
					url : 'distribu.do?method=addSettlementYf',
					...
					success : function(msg) {		
						if (msg.flag) {
							$pjq.messager.alert('配送费结算', '配送费结算成功', 'info');
							$dialog.dialog('close');
			    ...
			    $pjq.messager.alert('配送费结算', '必填信息不可为空', 'info');
			  }
	};	
    ...
    <form action="" method="post"  id="Regform">
 	<table class="tableclass">
			<tr>
					...
					<input type="radio"  name="type" value="3"  style="width:14px" onclick="sk(this)"/><label  style="margin-left: 10px">刷卡</label>
					<input type="radio"  name="type" value="2"  style="width:14px" onclick="zp(this)"/><label  style="margin-left: 10px">油卡</label>				
					</td> 
				</tr>
			...
			<tr id="yhkh"  style="display: none;">
				...
				<td ><font color="red" style="margin-right:10px">*</font>油卡编号:</td><!--  style="color: blue;"-->
