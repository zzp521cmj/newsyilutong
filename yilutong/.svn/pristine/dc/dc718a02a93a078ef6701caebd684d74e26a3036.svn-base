...
.combo {
  ...
  <div id="dialog">
  	...
  	<input type="hidden" id="updateid" name="customer_id">
  		<input type="hidden" id="departmentid">
		<span id="bigmapleave">添加客户</span> <img id="sm" onclick="closed(1)" alt="关闭"
		src="images/close.png" width="10px" height="10px" style="margin-top: 2px">
		<div align="center" style="margin-top: 15px">
		<label><font color="red">客户编码</font></label><input id="customer_num" type="text" name="customer_num"  class="validate[required]" style="margin-left: 10px">
		</div>
		<div align="center" style="margin-top: 15px">
		<label><font color="red">客户名称</font></label><input id="customer_name" type="text" name="customer_name"  class="validate[required]" style="margin-left: 10px">
		</div>
		<div align="center" style="margin-top: 15px">
		<label><font>联系方式</font></label><input id="mark_tel1" type="text" name="customer_tel" class="" style="margin-left: 10px">
		</div>
		...
		<div align="center" style="margin-top: 15px">
		<label style="margin-left: -12;margin-right: 10px">详细地址</label><textarea id="address1" name="customer_address"style="margin-left: 10px;resize: none;width: 173px;height: 30px;"></textarea>
		</div>
		<!-- <div align="center" style="margin-top: 15px">
  ...
  function initMap() {
	...
	//var name = document.getElementById("mark_name").value;
		...
		document.forms[0].action="remarkMap.do?method=updateRemark&&customer_point="+cpoint+"&&kkk="+data;
	...
	updatemark(data);
		...
		$.ajax({
  			type: "POST",
  			async : false,
  			url:'remarkMap.do?method=checkName',
  			data:{name:name},
  	...
  	//可提交
  		var cpoint = zpoint.lng+","+zpoint.lat;
  		myform.action="remarkMap.do?method=saveRemark&&customer_point="+cpoint+"&&kkk="+data;
  		myform.submit();
  		}else{
	...
	function updatepolygon(data){
		...
		$("#hidden_point").val(ppoint);
  		myform.action="remarkMap.do?method=updateRemark&&remark_state=polygon&&kkk="+data;
  		myform.submit();
  		}
	}
	...
	ppoint=zpoint.lng+","+zpoint.lat+"|";
		...
		$("#hidden_point").val(ppoint);
  		myform.action="remarkMap.do?method=saveRemark&&remark_state=polygon&&kkk="+data;
  		myform.submit();
  		}
	}
  ...
  var department;
  	$.ajax({
  			type: "POST",
  			async : false,
  			url:'remarkMap.do?method=getRemark',
  			data:{id:data},
  			success:function(data){
  					$.ajax({
  					type: "POST",
  			...
  			if(1==1){
  			$("#updateid").val(data.customer_id);
	  			var remark_point = data.customer_point.split(",");
			...
			var addComp = rs.addressComponents;
			 infoWindow = new BMap.InfoWindow("<label class='la'>客户编号:"+data.customer_num+"<br>客户名称:"+data.customer_name+"</br>地址:"+data.customer_address+
			 "</br>联系方式:"+data.customer_tel+"</label>",opts2);  // 创建信息窗口对象 
				$("#address1").val(data.customer_address);
				map.openInfoWindow(infoWindow,point);
				marker.addEventListener("click", function(){          
				map.openInfoWindow(infoWindow,point); //开启信息窗口
			});
		...
		marker.addEventListener("dragend",function(e){
			...
			//circle.setCenter(e.point);
			 infoWindow.setContent("<label class='la'>客户编号:"+data.customer_num+"<br>客户名称:"+data.customer_name+"</br>地址:"+data.customer_address+
			 "</br>联系方式:"+data.customer_tel+"</label>");  // 创建信息窗口对象 
			...
			$("#mark_tel1").val(data.customer_tel);
			$("#dialog").slideDown(500,function(){});
