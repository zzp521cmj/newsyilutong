  ...
  var pids=[];
 	pids.push(pid);
  	$(function(){
  		...
  		$.ajax({
  		  	url: 'settleDFYF_fu.do?method=getSettlout',
  		  	data:{id:pids.join(",")},
  		  	success:function(data){
  		  		data.back_fee=parseFloat(data.back_fee)-parseFloat(data.settlds_fu.split(",")[1]);
  		  		data.paid_fu=parseFloat(data.settlds_fu.split(",")[0])-parseFloat(data.settlds_fu.split(",")[1]); 	
 	 ...
 	 //可提交
		$pjq.messager.confirm('运费结算','确定要提交吗?',function(r){   
	    	if (r){    
				$.ajax({
					type: "POST",
		  			url:'settleDFYF_fu.do?method=settlementYfTG',
		  			data:{
		  			ids:pids.join(","),
		  			notes:$("#remarks").val()
		  			},/* $('#Editform').serialize() */
		  			success:function(msg){
		  				if(msg.flag){
		  					$pjq.messager.alert('结算审核','运单运费结算审核通过成功','info');
 	 ...
 	 //可提交
		$pjq.messager.confirm('运费结算','确定要提交吗?',function(r){   
	    	if (r){    
				$.ajax({
					type: "POST",
		  			url:'settleDFYF_fu.do?method=settlementYfBH',
		  			data:{
		  			ids:pids.join(","),
		  			notes:$("#remarks").val()
		  			},/* $('#Editform').serialize() */
  	...
  	<table class="tableclass">
		...
		<tr>
	 	<td><font color="red" style="margin-right:10px">*</font>运单号:</td>
			...
			<td><font color="red" style="margin-right:10px">*</font>应付运费:</td>
			<td class="td2">
				<input type="text" class="validate[required]" 	readonly="true" id="price" name="back_fee" />
	      	</td>
	    </tr>
	    <tr>			
			<td><font color="red" style="margin-right:10px">*</font>实付运费:</td>
			<td class="td2">
				<input type="text" class="validate[required]" readonly="true" id="price1" name="paid_fu" />
	      	</td>
	      	<td>			
