...
<html>
  ...
  var submitFormEditss=function($dialog, $grid, $pjq, $mainMenu) {
		...
		$pjq.messager.confirm('代收款结算', '确定要结算提交吗?', function(r) {
			if (r) {
				$.ajax({
					type : "POST",
					url : 'settleDaiskgf.do?method=addSettlementYf',
					...
					success : function(msg) {		
						if (msg.flag) {
							$pjq.messager.alert('代收款结算', '代收款结算成功', 'info');
							$dialog.dialog('close');
			    ...
			    $pjq.messager.alert('代收款结算', '必填信息不可为空', 'info');
			  }
	};	
