...
(function() {
	...
	(function() {
		...
		c.dom.getStyle = function(j, i) {
			var l = c.dom;
			j = l.g(j);
			var k = j.style[i] || (j.currentStyle ? j.currentStyle[i] : "")
					|| l.getComputedStyle(j, i);
			return k
		};
		...
		c.event.on = function(j, m, o) {
			...
			var n = function(q) {
				o.call(j, q)
			}, i = c.event._listeners, l = c.event._eventFilter, p, k = m;
			m = m.toLowerCase();
			if (l && l[m]) {
				p = l[m](j, m, n);
				k = p.type;
		...
		c.event.un = function(k, n, j) {
			...
			var q = c.event._listeners, l = q.length, m = !j, p, o, i;
			while (l--) {
				p = q[l];
	...
	f.prototype._popUpDiv = function(E, u) {
		...
		c.event.on(u, "onclick", function() {
			n()
		});
		c.event.on("tcClose", "click", function(G) {
			n()
		});
		function n() {
			if (!C.bShow()) {
		...
		function j() {
			...
			E.timer = setInterval(function() {
				b(G + (new Date()).getTime(), function() {
					var L = f.curTime;
					var K = L.getHours();
					var M = (K < 10 ? ("0" + K) : K)
							+ ":"
							+ (L.getMinutes() < 10 ? ("0" + L.getMinutes()) : L
									.getMinutes());
					w.innerHTML = M;
					E.hideTraffic();
					E.showTraffic()
		...
		function s() {
			...
			G
					.push('<div class="maplibTcColor" id="tcTitle">&#x5B9E;&#x65F6;&#x8DEF;&#x51B5;</div>');
		...
		function B(H) {
			c.event.on("tcViewPrediction", "click", function() {
				if (y) {
					G()
				} else {
					j()
			...
			c.event.on("tcUpdate", "click", function() {
				j()
			})
		}
		function p() {
		...
		function l(G) {
			c.event.on("tcWeek", "onclick", function(J) {
		...
		function D(K) {
			...
			c.on("tcPrev", "click", function() {
				N("prev")
			});
			c.on("tcNext", "click", function() {
				N("next")
			});
			function N(P) {
				var Q = t;
	...
	function b(j, k) {
		...
		if (i.addEventListener) {
			i.addEventListener("load", function(m) {
				var l = m.target;
				...
				i
						.attachEvent(
								...
								function(m) {
									...
									if (l
											&& (l.readyState == "loaded" || l.readyState == "complete")) {
										l.parentNode.removeChild(l);
										if (k) {
											k()
										}
