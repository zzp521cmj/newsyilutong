...
public interface SettleDaiskgfDao {
	//结算运费修改
	int updateSettlementYf(String id,String transpfrees);
	//结算订单修改
	int addSettlementYf(@Param("array")String[] id,String transpfrees,String transpfree_user,String transpfree_remaks,String khbank,String usernames,String kahao,String zpnum );
	//运费结算提审
	int delSettlementYf(@Param("array")String []ids,String transpfree_sh_dids,String transpfree_th_users,String transpfree_th_notes);
	//运费审核
	...
	//结算运费审核历史记录查询
		...
		//结算历史表修改查询
		List<Settle_historyGF>	getupdateSettle(String table_name,Integer  settl_order,@Param("array")String[] idss);
		//结算历史表审核状态修改
		int 	updateSettle(@Param("list")List<Settle_historyGF> list,String table_name,String settl_state);
}
