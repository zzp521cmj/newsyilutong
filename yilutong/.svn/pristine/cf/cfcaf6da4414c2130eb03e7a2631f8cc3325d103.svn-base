...
import com.jy.service.impl.SettlementGFServiceImpl;
import com.jy.service.impl.SettlementTCServiceImpl;

/*
 * 运费结算
 * */
@Controller
@RequestMapping(value = "/settleTC_fu.do")
public class SettlementTCController {
	
	@Resource
	private SettlementTCServiceImpl bid;
	@Resource
	private SettleDaiskgfService sid;
	...
	public String setlRcordInfo(HttpServletRequest request,String pid,String state){
		...
		return "settlementTC/setlRcordInfo";
	}
	//运费结算审核历史记录主页面settlementGFSH
		...
		public String settlementShRC(HttpServletRequest request,String pid){
			request.setAttribute("pid", pid);
			return "settlementTCSH/shrcordInfo";
		}
	//运费结算审核主页面
	@RequestMapping(params="method=settlementDaiskSh")
	public String settlementGFSH(){
		return "settlementTCSH/settlementDsShInfo";
	}
	//运费结算主页面
	@RequestMapping(params="method=settlementTcPage")
	public String getSettlementDsSearchPage(HttpServletRequest request,String agreement_id){
		request.setAttribute("pid", agreement_id);
		return "settlementTC/settlementDsInfo";
	}
	
	//协议查询页面 customerSettlInfo.jsp
	@RequestMapping(params="method=customerSettlInfo")
	public String customerSettlInfo(HttpServletRequest request,String pid){
		request.setAttribute("pid", pid);
		System.out.println("进入");
		return "settlementTC/customerSettlInfo";
	}
	//跳转到提审页面
	@RequestMapping(params="method=thSettlement")
	public String thSettlement(HttpServletRequest request,String []pid){
		request.setAttribute("pid", pid);
		return "settlementTC/thSettlement";
	}
	//跳转到单个审核页面
...
//		System.out.println(json.toString());
	...
	public Map<String,Object> saveSettlementYf(String[] dds,String type, String remarks,String khbank,String usernames,String kahao,String zpnum,String userid,String state) {	
		...
		for(int i=0;i<order.size();i++){
			...
			if(order.get(i).getPaytypes().equals("0")){
				sh.setAllmoney(order.get(i).getChange_pay());				
			}else{
		//		sh.setAllmoney(order.get(i).getSettltc_fu().split(",")[1]);
				sh.setAllmoney(Float.parseFloat(order.get(i).getChange_pay())-Float.parseFloat(order.get(i).getSettltc_fu().split(",")[1])+"");
			}
			sh.setOrder_id(order.get(i).getShiping_order_id());	
			if(order.get(i).getPaytypes().equals("0")){
				sh.setSettl_money(order.get(i).getPaid_fu());				
			}else{
				sh.setSettl_money(Float.parseFloat(order.get(i).getPaidtc_fu())-Float.parseFloat(order.get(i).getSettltc_fu().split(",")[0])+"");
			}
			...
			list.add(sh);
			if(order.get(i).getPaytypes().equals("0")){
				order.get(i).setSettltc_fu(order.get(i).getPaidtc_fu()+","+0);		
			}else{
				order.get(i).setSettltc_fu(order.get(i).getPaidtc_fu()+","+order.get(i).getSettltc_fu().split(",")[0]);
			}
			
			//order.get(i).setSettltc_fu(order.get(i).getPaidtc_fu()+","+order.get(i).getTrade_cha());
		}
		Map< String, Object> map =new HashMap<String,Object>();
		map.put("list", list);
		map.put("order", order);
	...
	Json addSettlementYf(String[] dds,String type, String remarks,String khbank,String usernames,String kahao,
		...
		Json json = new Json();
		//保存结算记录
		bid.saveSettlchecks((List<Settle_historyGF>) saveSettlementYf(dds, type, remarks, khbank, usernames, kahao, zpnum, user.getId(),"1").get("list"));
	...
	Json delSettlementYf(String[] dds,String did,String notes,HttpServletRequest request) {
		User user=(User) request.getSession().getAttribute(SessionInfo.SessionName);
		Json json = new Json();
		List<Settle_historyGF> list=sid.getupdateSettle("jy_settlemengf_history", 1, dds);//最新结算记录
		sid.updateSettle(list,"jy_settlemengf_history","1");//修改结算记录审核状态
...
//			System.out.println(json.toString());
	...
	Json shSettlementYfTG(String[] ids,String notes,HttpServletRequest request) {
		User user=(User) request.getSession().getAttribute(SessionInfo.SessionName);
		Json json = new Json();
		List<Settle_historyGF> list=sid.getupdateSettle("jy_settlemengf_history", 1, ids);//最新结算记录
		sid.updateSettle(list,"jy_settlemengf_history","2");//修改结算记录审核状态
	...
	Json shSettlementYfBH(String[] ids,String notes,HttpServletRequest request) {
		...
		for(int i=0;i<order.size();i++){			
				order.get(i).setSettltc_fu(order.get(i).getSettltc_fu().split(",")[1]+","+order.get(i).getSettltc_fu().split(",")[1]);
		}
		bid.saveSettl_fu(order);
		
		
		List<Settle_historyGF> list=sid.getupdateSettle("jy_settlemengf_history", 1, ids);//最新结算记录
		sid.updateSettle(list,"jy_settlemengf_history","3");//修改结算记录审核状态
...
//				System.out.println(json.toString());
			...
			public @ResponseBody Map<String,Object> getCustomerSetYf(String rows, String page,String agreement_number, String agreement_type,String agreementstartdate,String agreementenddate,String sort,String order) {
					//当前第几页
				Integer page1 = 1;
							//当前页一共几条
							Integer rows1 = 0;
					...
					System.out.println("++++协议id====="+agreement_type);
					List<CreateAgreement> list = bid.getCustomerSetYf((page1 - 1) * rows1,rows1,agreement_number, agreement_type,agreementstartdate,agreementenddate,sort,order);
					int count = bid.getCustomerSetYfs(agreement_number, agreement_type,agreementstartdate,agreementenddate,sort,order);
					Map<String, Object> map = new HashMap<String, Object>();
					map.put("total", count);
					map.put("rows", list);
//					JSONObject json=new JSONObject();
...
//					System.out.println(json.toString());
			...
			public @ResponseBody List<ShippingOrder> getCustomerSetYfout(HttpServletResponse response,String[] headtitle, String[] fieldName,String customer_name, String agreement_number,String agreement_type,String agreementstartdate,String agreementenddate,String sort,String order, String [] checkarray){
				String filename = "中转协议信息导出";	
				List<CreateAgreement> list;
				System.out.println("====="+checkarray.length);
				System.out.println("======"+agreement_number);
				if(checkarray.length>0){
					list= bid.getCustomerSetYfout(null, null,null,null,null,null,checkarray);					
				}else{
					list= bid.getCustomerSetYfout(agreement_number,agreement_type,agreementstartdate,agreementenddate,sort,order,null);
				}					
				try {			
					// 声明一个工作薄
					HSSFWorkbook workbook = new HSSFWorkbook();
