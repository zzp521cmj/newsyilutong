  ...
  <script type="text/javascript">
 		...
 		var grid;
  		...
  		$(function(){
			...
			$("#flag").combobox("setValue", "4");
  			...
  			grid=$("#dg").datagrid({
  			url : 'settleDFYF_fu.do?method=getSettlementYfSHInfo&isdelivery='+did,
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		rowStyler: function(index,row){
  			...
  			columns : [[
			  	...
			  	{field : 'settleds_sh',title : '审核状态', width : 80,align :'center',sortable:'true',
  				...
  				{field : 'paid_fu',title : '实付运费总额',width : 100,align:'center'	/* sortable: true */},
	...
	var ids=[];
  		var row=$("#dg").datagrid('getSelections');
		  	...
		  	if(ids.length>0){
	  			dialog = parent.jy.modalDialog({
				title : '结算审核',
				url : 'settleDFYF_fu.do?method=shSettlement&pid=' + ids.join(","),
				width : 600,
				height:420,
				buttons : [{
					text : '<input type="button" class="btncss" value="通过"/>',
	...
	//查看审核详情
  		function OrderUpdates(ids){
  			dialog = parent.jy.modalDialog({
			title : '结算审核记录',
			url : 'settleDFYF_fu.do?method=settlementShRC&pid='+ids,
/* 				 buttons :'#addbuttons'
*/
			width : 725,
			height:550,
