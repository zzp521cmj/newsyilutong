...
<html>
  ...
  <script type="text/javascript">
 		...
 		var grid;
  		$(function(){
  			grid=$("#dg").datagrid({
  			url : 'waybill.do?method=getShipManual',
  			border : false,
  			rownumbers : true,
  			fit : true,
		    ...
		    rowStyler:function(index,row){
					if (row.shipping_order_state==0){
								return 'color:black';
						}else if(row.shipping_order_state==1){
						  return 'color: purple';
						}else if(row.shipping_order_state==2){
						  return 'color: red';
						}else if(row.shipping_order_state==3){
						  return 'color: green';
  		...
  		/* 	remoteSort:true, */
  			...
  			{field : 'shipping_order_state',title : '运单状态',width : 70,align:'center',
  				...
  				{field : 'duty_rate',title : '税率', width : 80,align :'center',sortable:'true'},
  	...
  	function putgouxuan(){
		...
		for(i=1;i<allRows2.length;i++)
		{
  		...
  		for(var i=0;i<array.length;i++){
  		 ...
  		 $("#ShippingOrderid").val(checkarray.join(","));
  		var formout=$("#formout");
  		formout.submit();		
  		}else{
...
*/
  ...
  <body class="easyui-layout">
  	<form action="waybill.do" id="outputform" method="post">
    	<div region="north" title="检索" class="st-north" border="false"  height="60px" collapsible="false"  >
	    	<lable>开单时间:</lable>
			<input  id="start_time" name="start_time" class="Wdate"  readonly="readonly" style="width:120px;height:22px; " onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
			-
			<input id="end_time" name="end_time" class="Wdate"  readonly="readonly" style="width:120px;height:22px;" onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
	    	
	    	<lable >运单编号:</lable>
	    	 <input type="text" class="search-text" id="ddId"  name="ddId" style="width:120px; margin-right:0px"  >
	    	 <lable>交接方式:</lable>
		 ...
		 <lable >发货人:</lable>
	    	<input type="text"  class="search-text"style="width:120px" name="custom_name"  id="custom_name" >
	    	 ...
	    	 <lable>运行区间:</lable>
			<input  id="send_station"  name= "send_station" class="search-text" style="width:120px;height:22px;margin-right:0px" />
			-
			<input id="end_address" name="end_address" class="search-text"style="width:120px;height:22px;margin-right:0px" />
	    	<lable>运单状态:</lable>
				<select class="easyui-combobox" id="order_state" name="order_state"
					style="width:120px;"
					data-options="panelHeight : 'auto',editable:false">
					<option id="" value="">全部</option>
	    	...
	    	<select class="easyui-combobox" id="pay_type" name="pay_type"
	    	 ...
	    	 <lable >收货人:</lable>
	    	<input type="text"  class="search-text"style="width:120px" name="receipt"  id="receipt" >
	    	
	    <lable>品名:</lable>
			<input type="text" class="search-text" id="goods_name"style="width:120px"  name="goods_name" >    
			</div>
    	</div>
    	<div region="center" >
    	...
    	<div title='签收' style='height: 16px;background:blue;-moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;    float: right;
		    	margin: 5 1%;color: white;
		    padding-left: 10px;
		    padding-right: 10px; '><span>签收</span></div>
		    <div title='已到达' style='height: 16px;background:green;-moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;    float: right;
		    	margin: 5 1%;color: white;
		    padding-left: 10px;
		    padding-right: 10px; '><span>已到达</span></div>
		    	  <div title='装运中' style='height: 16px;background:red;-moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;    float: right;
		    	margin: 5 1%;color: white;
		    padding-left: 10px;
		    padding-right: 10px; '><span>装运中</span></div>
		    <div title='配送中' style='height: 16px;background:purple;-moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;    float: right;
		    	margin: 5 1%;color: white;
		    padding-left: 10px;
		    padding-right: 10px; '><span>配送中</span></div>
		  <div title='未出库' style='height: 16px;background:black;-moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;    float: right;
		    	margin: 5 1%;color: white;
		    padding-left: 10px;
		    padding-right: 10px; '><span>未出库</span></div>
		...
		<input type="hidden" name="method" value="outShipManual">
		<input type="hidden" id="fieldName" name="fieldName">
		<input type="hidden" id="headtitle" name="headtitle">
		</form>
		 <form action="waybill.do" target="_blank" id="formout" method="post">
		 	<input type="hidden" name="method" value="outShipManuals">
		 	 <input type="hidden" id="fieldName1" name="fieldName1">
	 		<input type="hidden" id="headtitle1" name="headtitle1">
		 	<input type="hidden" id="ShippingOrderid" name="ShippingOrderid">
		 </form>
  </body>
</html>