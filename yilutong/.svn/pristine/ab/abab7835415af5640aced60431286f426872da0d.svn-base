...
<html>
  ...
  <script type="text/javascript">
 		...
 		var id = '<%=pid%>';
  		$(function(){
  			grid=$("#dg").datagrid({
  			url : 'Destination.do?method=getShippingAuditing&pid='+id,
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
...
/* 				if (row.destination_stata==0) {
  			...
  			columns : [[
  				...
  				{field : 'send_time',title : '发车时间',width : 60,align:'center'},
  				...
  				{field : 'send_phone',title : '发货联系电话',width : 80,align:'center',sortable:'true'},
  			...
  			//	{field : 'receipt_phone',title : '收货人手机',width : 80,align:'center'},	  				
  				{field : 'destination_fee',title : '应收代收运费',width : 80,align:'center',
  				   formatter:function(val,row,index){
  				...
  				{field : 'paid_fee',title : '实收代收运费',width : 80,align:'center'},
  				{field : 'chayi',title : '差异',width : 80,align:'center',
  					formatter:function(val,row,index){     		   
	              	    return Number(Number(row.destination_fee*1)-Number(row.paid_fee*1));	           		   
  					}
  				}, 							
  			...
  			/* 	formatter:function(val,row,index){
  		     ...
  		     /*  	 formatter:function(val,row,index){
  			    ...
  			    {field : 'destination_stata',title : '结算方式',width : 80,align:'center',
  			    formatter:function(val,row,index){
  				if(val==2 ){
  						if(val==2 ){
  						return "转账";
  			...
  			{field : 'des_tz_stata',title : '调整',width : 80,align:'center',
  			formatter:function(val,row,index){
  				if(val==2 ){
  						return "调整已结算";
  						}else if(val==1){
  			...
  			{field : 'des_sh_stata',title : '审核状态',width : 80,align:'center',
  			formatter:function(val,row,index){
  					if (val == 0) {
							return "未审核";
						} else if (val == 1) {
  			...
  			{field : 'department_name',title : '部门名称',width : 80,align:'center'},
  			{field : 'settlyf_time',title : '结算日期',width : 80,align:'center'},
  			{field : 'username',title : '结算人',width : 80,align:'center'},			
  			{field : 'settlyf_remaks',title : '结算备注',width : 80,align:'center'},
  			{field : 'settlyf_th_times',title : '提审日期',width : 80,align:'center'},
  			{field : 'usernameth',title : '提审人',width : 80,align:'center'},
  			{field : 'settlyf_th_remaks',title : '提审备注',width : 80,align:'center'},
	...
	btnMoreSearch.click(function() {
		...
		for ( var i = 0; i < array.length; i++) {
			...
			if (id != "") {
				...
				title : '费用结算',
				type : "POST",
				url : 'Destination.do?method=getEditCompanyPagef&pid='+id+'&fee='+fee,
				//data :{fee:fee},
				width : 300,
				height:200,
				buttons : [{
  ...
  function compute(colName) {
    ...
    function CheckManager(pid){
        dialog = parent.jy.modalDialog({
			title : '费用结算',
			url : 'Destination.do?method=getShippingshenhe&pid='+pid,
			width : 750,
			heght: 600,				
		});
    }	
 ...
 document.onkeydown = function(event) {
	...
	/* 	  				if (row[i].customs_recheck_state==0&&row[i].write_off!=3) {
			  ...
			  if(ds == false){
		  			dialog = parent.jy.modalDialog({
					title : '结算提审',
					url : 'Destination.do?method=getArraignedClose&pid=' + ids.join(","),
					width :450,
					height:300,
					buttons : [{
						text : '<input type="button" class="btncss" value="确定"/>',
...
//审核
  	function shenhetijiao(){
		...
		/* 	if (array[i].company_id !=0) {
			...
			if (j==1) {
	  			dialog = parent.jy.modalDialog({
				title : '费用结算',
				url : 'Destination.do?method=getDriverCh&id=' + id.join(","),
				data : {				
				},
				width : 700,
				height: 450,
  			...
  			$.messager.alert("客户结算"," 请选择审核中的信息","info");
  		} 	
  		}else{
  ...
  <form action="Destination.do" id="outputform" method="post">
	    ...
	    <lable class="divlabel">发车日期:</lable>
	    	 <input  id="send_time"  class="Wdate"    name=" send_time"  style="width:150px;height:22px; "onfocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
	    	 -
	    	 <input  id="send_times"  class="Wdate"   name=" send_times" style="width:150px;height:22px; "onfocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
	    	 <lable class="divlabel">运单编号:</lable>
	    	 <input type="text" id="shipindi"  name="shipindi"class="search-text" style="width:150px;height:22px;"/>
			<label class="divlabel">客户名称:</label>
			 <input type="text" id="jiesuStata"  name="jiesuStata"class="search-text" style="width:150px;height:22px;"/>
	    	 ...
	    	 <input id="jiesuantime"  class="Wdate"   name="jiesuantime" style="width:150px;height:22px; "onfocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
	    	 -
	    	 <input  id="jiesuantimes"  class="Wdate"  name="jiesuantimes" style="width:150px;height:22px; "onfocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
			 <label class="divlabel">制单日期:</label> 
	    	 <input  id="order_time"  class="Wdate"  name="order_time"  style="width:150px;height:22px; "onfocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
	    	 -
	    	 <input  id="order_times"  class="Wdate"   name=" order_times"style="width:150px;height:22px; "onfocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
			</div>
			</div>
    	<div region="center" >
    	...
    	<div id="tb">
    	<a href="javascript:void(0)" class="easyui-linkbutton"  data-options="iconCls:'icon-edit',plain:true"onclick="shenhetijiao()">审核</a>
		...
		<input type="hidden" name="method" value=getCustomerInputOut>
