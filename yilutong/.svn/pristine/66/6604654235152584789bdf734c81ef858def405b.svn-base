...
<html>
  ...
  <script type="text/javascript">
		...
		var id ='<%=id%>';
		 $.ajax({
			url : 'payable.do?method=getshipinCollect',
			data : {
				id : id
			},
  ...
  if($("#Regform").validationEngine('validate')&&$("#receiptid").select2("val")!=""){	
		...
		$pjq.messager.confirm('结算审核', '确定要审核吗?', function(r) {
			if (r) {
				$.ajax({
					type : "POST",
					url : 'payable.do?method=updateCheckCollect',
  ...
  if($("#Regform").validationEngine('validate')&&$("#receiptid").select2("val")!=""){	
		...
		$pjq.messager.confirm('结算审核', '确定要驳回吗?', function(r) {
			if (r) {
				$.ajax({
					type : "POST",
					url : 'payable.do?method=updateCheckReject',
					data :{id:pid.join(","),close_recheck_remarks:remarks,username:name,change_fee1:change_fee1,change_pay:change_pay},
					success : function(msg) {		
						if (msg.flag==true) {
 	      ...
 	      <table class="tableclass">
              <tr>
			    <td>				
				    ...
				    <td >实付中转费：</td>
					<td  class="td2"><input placeholder="0.00"  class="validate[custom[number],max[99999.99],min[0.00],custom[dd]]"  type="text" name="change_fee1" id="paid_fee" disabled="disabled"/> 
					</td>
				</tr>
				<tr>
				    <td >应付中转费：</td>
					<td  class="td2"><input placeholder="0.00"  class="validate[custom[number],max[99999.99],min[0.00],custom[dd]]"  type="text" id="transport_pay" name="change_pay" disabled="disabled"/> 
					</td>
					 <td >实收中转费：</td>
					<td  class="td2"><input placeholder="0.00"  class="validate[custom[number],max[99999.99],min[0.00],custom[dd]]"  type="text" id="paid_chayi" name="paid_chayi" disabled="disabled"/> 
					</td>
				</tr>
				<tr>
