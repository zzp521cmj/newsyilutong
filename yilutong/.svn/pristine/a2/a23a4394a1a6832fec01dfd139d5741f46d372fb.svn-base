...
// 保存线路并新建s
function bd() {
	var cs=$("#car").select2('val');
	var linid=$("#linid").val();
	var carnos=new Array();
	var data=$("#car").select2('data');
    $.each(data, function (key, val) {
    	carnos.push(val.text);
    });
	if(carnos.length>0){
		$.ajax({
			type : "POST",
			url : "maplin.do?method=bdcar",
			data : {
				cs : cs.join(","),
				cns : carnos.join(","),
				linid :linid
			...
			success : function(data) {
				if (data.flag) {
					...
					$.messager.alert("绑定线路", "绑定线路失败",'info');
				}
			}
		});
	  }else{
	    $.messager.alert('绑定线路', '未选择车辆', 'info');
	  }
}// 保存线路并新建e
...
function bdandadd() {
	...
	$.ajax({
		type : "POST",
		url : "maplin.do?method=deleteallcar",
		data : {
			linid :linid
		},
		success : function(data) {
			if (data.flag) {
				$.messager.confirm('绑定线路', '清空绑定车辆成功', function(r) {
					if(r){
						backto();
					}else{
						 $("#car").select2("val","");
				...
				$.messager.alert("绑定线路", "清空绑定车辆失败,未绑定车辆的路线无法清空",'info');
			}
		}
	});
}
...
function setaddress(e){//设置mark的地址
	point = e.target.getPosition();// point
	var opts = {
			  width : 130,
			  enableAutoPan : true, // 自动平移
			  title : "<span style='color:#D2693C;font-size: 14px;font-weight: bold'>设置起止城市</span>" , // 信息窗口标题
			  enableMessage:false,//设置允许信息窗发送短息
			  offset : new BMap.Size(0, -20)
			};
			var infoWindow = new BMap.InfoWindow("<input type='button' value='设为开始城市' style='height: 30;border: 1px solid #ccc;background-color: white;cursor: pointer; ' onclick='setcity("+ '"'+ point.lng+ '"'+","+'"'+ point.lat+'"'+ ")'  onmouseover='over(this)' onmouseout='out(this)'><input type='button' value='设为结束城市'  style='height: 30;border: 1px solid #ccc;background-color: white;margin-left: 20px;cursor: pointer;' onclick='setecity("+ '"'+ point.lng+ '"'+","+'"'+ point.lat+'"'+ ")'  onmouseover='over(this)' onmouseout='out(this)'>", opts);  // 创建信息窗口对象 
