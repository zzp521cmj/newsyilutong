  ...
  <script type="text/javascript">
 		...
 		var grid;
  		...
  		$('#isdelivery').val(did);		
			...
			$("#flag").combobox("setValue", "4");
  			...
  			grid=$("#dg").datagrid({
  			url : 'settleDaisk.do?method=getSettlementYfSHInfo&isdelivery='+did,
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		rowStyler: function(index,row){
  			...
  			columns : [[
			  	...
			  	{field : 'setldaisk_sh',title : '审核状态', width : 80,align :'center',sortable:'true',
  				...
  				/* formatter:function(val,row,index){			
		  					if(isNaN(parseFloat(row.paid_fu))){
					  		 val=row.back_fee;
					  		}else{
					  		 val=parseFloat(row.back_fee)-parseFloat(row.paid_fu); 	
					  		}	  		
  							return val;		
  					} */
	...
	var ids=[];
  		var row=$("#dg").datagrid('getSelections');
		  	...
		  	if(ids.length>0){
	  			dialog = parent.jy.modalDialog({
				title : '结算审核',
				url : 'settleDaisk.do?method=shSettlement&pid=' + ids.join(","),
				width : 600,
				height:420,
				buttons : [{
					text : '<input type="button" class="btncss" value="通过"/>',
				...
				},
				 {
  				...
  				$.messager.alert("运费结算","请选择已提审信息","info");
  		}			  		
	...
	//查看审核详情
  		function OrderUpdates(ids){
  			dialog = parent.jy.modalDialog({
			title : '结算审核记录',
			url : 'settleDaisk.do?method=settlementShRC&pid='+ids,
/* 				 buttons :'#addbuttons'
*/
			width : 725,
			height:550,
