...
public class Account_payableController {
		...
		@Resource
		private Account_payableDao ado;
		
		/*
		 * 跳转到应付款页面
		 */
		@RequestMapping(params="method=tym")
		public String tym(){
			return "expense_report/account_payable";	
		}	
		...
		Map<String, Object> dueselect(String rows, String page,
			...
			List<ShippingOrder> list = ado.dueselect((page1 - 1) * rows1, rows1, custom_name, receipt, shiping_order_num,close_recheck);
		...
		String inquireName(String custom_name) {
			List<Customer> flist = ado.inquireName(custom_name);
			JSONObject jb = null;
		...
		String inquireName1(String receipt) {
			List<ShippingOrder> flist = ado.inquireName1(receipt);
			JSONObject jb = null;
			JSONArray jsons = new JSONArray();
		 ...
		 */
	           ...
	           List<ShippingOrder> outShi(HttpServletResponse response,String[] headtitle,
					...
					String filename = "应付款导出";	
					List<ShippingOrder> list = ado.outShi(checkarray);
