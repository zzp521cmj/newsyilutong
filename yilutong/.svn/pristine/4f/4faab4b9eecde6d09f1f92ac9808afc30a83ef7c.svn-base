  ...
  <script type="text/javascript">
 		...
 		var editval;
  		...
  		$('#dlg1').dialog({
			...
			$("#kname").val('<%=flg%>');
  			grid=$("#dg").datagrid({
  			url : 'settlementYf.do?method=getSettlementYf&kname='+'<%=flg%>',
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		rowStyler: function(index,row){
  			if (row.settle_tz!=2||row.is_settlementyf==0||row.settlyf_sh==3||(row.settlyf_sh==2&&(parseFloat(row.transport_pay)!=parseFloat(row.reality_sum)))){
					return 'color:blue;';
				}else if (row.settlyf_sh==0){
					return 'color:rgb(221, 14, 211);';
				}else if(row.settlyf_sh==1){
					return 'color:red;';
				}
			},
  			columns : [[
  				...
  				{field : 'transport_pay',title : '应收运费总额',width : 100,align:'center'/* sortable: true */},
  				{field : 'reality_sum',title : '实收运费总额',width : 100,align:'center'
  						/* ,editor:{
	                		type : 'numberbox',
		  				    options : {
		  				    precision : '2',
  				...
  				{field : 'settle_tz',title : '结算调整', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  						 if(val==1){ 						
  						return "调整未结算";
  						}else if(val==2){ 						
  					...
  					{field : 'settlyf_sh',title : '是否审核', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  						if(val==0||val==null){
  							return "未审核";
  						}else if(val==1){ 						
  				...
  				{field : 'dn',title : '提审部门',width : 90,align:'center'},  
  				{field : 'is_settlementyf',title : '是否结算', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  							if(val==0){
  							return "未结算";
  						}else if(val==1){ 						
	  			...
	  			{field : 'transpfree_sh',title : '是否调整', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  							if(val==4){
  							return "未调整";
  						}else if(val!=4){ 						
  				...
  				{field : 'tf',title : '提付',width : 80,align:'center'},
  				{field : 'xf',title : '现付',width : 80,align:'center'},
  				{field : 'hdqf',title : '货到前付',width : 80,align:'center'},
  				{field : 'fhyj',title : '发货方月结',width : 80,align:'center'},
  				{field : 'shyj',title : '收货方月结',width : 80,align:'center'},
  				{field : 'hdfk',title : '货到付款',width : 80,align:'center'},
  	...
  	function settlementSh(){
  		...
  		var row=$("#dg").datagrid('getSelections');
  			if(row.length>0){
	  			...
	  			if(row[i].settlyf_sh==0&&row[i].settle_tz==2){
		  	...
		  	if(ids.length>0){
				...
				title : '结算提审',
				url : 'settlementYf.do?method=thSettlement&pid=' + ids.join(","),
				width :450,
				height:300,
				buttons : [{
		  ...
		  var wids=[];
		 	var row=$("#dg").datagrid('getSelections');
	  		...
	  		//&&(parseFloat(row[i].transport_pay)!=parseFloat(row[i].reality_sum)
		  			if(row[i].is_settlementyf==0||row[i].settle_tz==1){
			  				ids.push(row[i].shiping_order_id);	  				
		  				}		  			
		  			}	
		  if(ids.length>0){
				  dialog= parent.jy.modalDialog({
					title : '运费结算',
					url : 'settlementYf.do?method=edSettlement&pid='+ids.join(","),
		/* 				 buttons :'#addbuttons'
		*/
					width : 530,
					height: 270,
	...
	//调整金额
		function changemoney(){
			 var ids="";
				var row=$("#dg").datagrid('getSelections');
		  			if(row.length==1){
			  			for(var i=0;i<row.length;i++){			  			
			  			if(row[i].is_settlementyf==0||(row[i].settle_tz!=2)||row[i].settlyf_sh==3||(row[i].settlyf_sh==2&&(parseFloat(row[i].transport_pay)!=parseFloat(row[i].reality_sum)))){
			  				ids=row[i].shiping_order_id;	  				  								
			  				}	  		
				  		}
					if(ids!=""){
						$("#form").form('clear');			
						if(row[0].is_settlementyf==0||row[0].trade_cha==0){
			  					$("#yingshou").val(row[0].transport_pay);
			  				}else if(isNaN(parseFloat(row[0].reality_sum))){
			  					$("#yingshou").val(row[0].transport_pay);	
			  				}else{
			  					$("#yingshou").val(parseFloat(row[0].transport_pay)-parseFloat(row[0].reality_sum));
			  					}	  									
						$("#dlg1").dialog('open');
						}else{
		...
		function dealOrder(){
	  		var row=$("#dg").datagrid('getSelections');
	  		var reality_sum;
	  		if(isNaN(parseFloat(row[0].reality_sum))||(row[0].is_settlementyf==0&&row[0].settle_tz==0)){
	  		 reality_sum=parseFloat($("#shishou").val());
	  		}else{
	  		 reality_sum=parseFloat($("#shishou").val())+parseFloat(row[0].reality_sum);
	  		}	  		
				  			$.messager.confirm('确认','您确定要提交处理吗？',function(r){ 
			    			if (r){ 
			    				$.ajax({									
									type: "POST",
									url : 'settlementYf.do?method=changePrice',
				           			data :  {
  		...
  		function OrderUpdates(ids){
  			dialog = parent.jy.modalDialog({
			title : '运费结算记录',
			url : 'settlementYf.do?method=setlRcordInfo&pid='+ids+'&state='+0,
/* 				 buttons :'#addbuttons'
*/
			width : 690,
			height:500,
  ...
  <body class="easyui-layout">
  	<form action="settlementYf.do" id="outputform" method="post">
