...
<script type="text/javascript">
	...
	$(function() {
		grid = $("#dg")
				.datagrid(
						{
							url : 'collect.do?method=getAdorn_fee',
							border : false,
							rownumbers : true,
							fit : true,
							singleSelect : false,
							...
							onAfterEdit : function(index, row) {//编辑关闭后
								...
								$.ajax({
									type : "post",
									url : 'collect.do?method=otherUpdate',
									data : row
		...
		for ( var i = 0; i < array.length; i++) {
			...
			if (id != "") {
					...
					dialog = parent.jy
							.modalDialog({
								title : '费用核销',
								url : 'collect.do?method=getEditCompanyPages&pid='
										+ id,
								width : 500,
								height : 300,
								buttons : [ {
	...
	function searchCar_owner() {
		$("#dg").datagrid('load', {
			...
			others_date : $.trim($("#others_date").val()),
			write_off5 : $.trim($("#write_off5").val()),
	...
	function del() {
		...
		for ( var i = 0; i < array.length; i++) {
			...
			if (a== true) {
					...
					if (r) {
						$.ajax({
							url : 'collect.do?method=editOther&pid=' + id+'&ids=ids',
							type : "POST",
							data : {
								id:id.join(",")
							},
...
<body class="easyui-layout">
	<form action="shipOrder.do" id="outputform" method="post">
		<div region="north" title="检索" class="st-north" border="false"
			...
			<input id="others_date" class="Wdate"
				style="width:50px;height:22px; "
				onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd '})" />
				-
				<input id="aa" class="Wdate"
				style="width:50px;height:22px; "
				onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd '})" />
			<td>核销:</td>
					<td ><select id='write_off5'  name="write_off5"  style="width:50px;height:22px">
							<option> </option>
