...
var submitForm = function($dialog, $grid, $pjq) {
		$pjq.messager.confirm('修改发车计划表','确定要修改发车计划表吗?',function(r){   
	    	if (r){
				$.ajax({
					type: "POST",
...
var pid='<%=flg%>';
	if(pid!=null){
		$.ajax({
  		  	url: 'automaticPlan.do?method=getClassPlanEditInfo',
  		  	data:{pid:pid},
  		  	success:function(data){
  		  		//var data=$.parseJSON(data);
  		  		$('#editform').form('load', data);
  		  		$("#planname_s").select2("data", {id:data.classline_id, text:data.classline_name});
				...
				//机构
				$('#cc').combotree('setValue',data.organization_id);
				//发车时间
				$("#departure_timeId").val(data.departure_time.substring(0,16));
				if(data.regular_type==0){
					$("#datetype_2").attr("checked","checked");
					$("#weeklable").show();
