...
<html>
  ...
  <script type="text/javascript">  
  	var pid='<%=pid%>';
  		$(function(){
		 $("#dg").datagrid({
			url : 'settleDFYF_fu.do?method=getSettleShRc&ddids='+pid,
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
  			...
  			columns : [[
  				...
  				{field : 'settlyf_time',title : '审核操作时间',width : 130,align:'center',
  				...
  				{field : 'department_name',title : '提审部门',width : 100,align:'center'},
  				{field : 'settlyf_type',title : '审核类型',width : 80,align:'center',
  					formatter: function(value,row,index){
		  				if(value==1){
		  				return '审核操作';
		  				}else if(value==0){
  				...
  				{field : 'settlyf_state',title : '审核状态',width : 80,align:'center',
  					formatter: function(value,row,index){
		  				if(value==1){
		  				return '审核通过';
		  				}else if(value==0){
  				...
  				{field : 'usernamecz',title : '操作人',width : 65,align:'center'},//方位（m³）
  				{field : 'settlyf_remaks',title : '备注信息',width : 120,align:'center'},					
  			]],
  			pagination:true,//分页
		    pageSize:10,
   ...
   <form action="car_b_rec.do" id="outputform" method="post">
  	<div region="north" title="检索" class="st-north" border="false" collapsible="false"  >
   	<lable >审核日期:</lable>
    	...
    	<div region="center" >
    		<table id="dg" title="运费结算审核记录信息"></table>
    	</div>
    	<div id="tb">
