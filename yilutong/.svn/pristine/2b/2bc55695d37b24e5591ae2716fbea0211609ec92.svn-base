...
<script type="text/javascript">
	...
	/* 	var fs = $("#functionname").val().split(",");
		...
		grid = $("#dg").datagrid(
						{
							url : 'settlementYf.do?method=getCustomerSetYf',
							fit : true,
							rownumbers : true,
							//singleSelect: true,
							selectOnCheck : true,
							...
							onDblClickRow:function(rowIndex, rowData){
								  			var pid=rowData.customer_id;
												 dialog = parent.jy.modalDialog({
																				title : '运单信息',
																				url : 'settlementYf.do?method=settlementYfPage&pid=' +pid,
																			/* 	width : 1150,
																				height:550, */
																				fit:true,
																				buttons : [{
...
<body class="easyui-layout">
	...
	<div region="north" title="检索" class="st-north" border="false"
		...
		<lable class="divlabel">制单日期:</lable>
			<input  id="starts"  class="Wdate" name="starts" readonly="readonly" style="width:135px;height:22px; " onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
			-
			<input id="ends"  class="Wdate"  name="ends"  readonly="readonly" style="width:135px;height:22px;" onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd HH:mm:ss'})" />		 	
		<lable class="divlabel">客户名称：</lable>
		<input type="text" id="customer_name"   name ="customer_name"  style="width:140px" />
		<lable class="divlabel">客户编码：</lable>
		<input type="text" id="customer_num"  name ="customer_num" style="width:140px" />
	 <a class="easyui-linkbutton" onclick="searchCar_owner()" data-options="iconCls:'icon-search'" id="searchId"   style="margin-left: 10px;"">查询</a>
	</div>
