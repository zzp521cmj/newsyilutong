...
<html>
  ...
  <script type="text/javascript">
 		...
 		var editval;
  		$(function(){
  			grid=$("#dg").datagrid({
  			url : 'payable.do?method=getOther',
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
  			...
  			multiSort:true,
  				...
  				{field : 'other_remarks',title : '应付其他费备注',width : 120,align:'center',editor:{
	                	type:'text',
	          			     options:{
	                			
	                    	validType:'other_remarks'
	                	}}},
  				{field : 'payble_other',title : '应付其他费',width : 80,align:'center'},
  				{field : 'send_other',title : '实付其他费',width : 80,align:'center',editor:{
	                	type:'numberbox',
	                	options:{
	                		precision:2,
	                    	validType:'send_other'
	                	}
	           		},	formatter:function(val,row,index){
	           		
	           		   if(val==null){
	           		 		return row.payble_other;
	           		   }else{
	           		   return row.send_other;
	           		   }
  					}
  				},
  				{field : 'other_remarks1',title : '应付其他费核销备注',width : 120,align:'center',editor:{
	                	type:'text',
	          			     options:{
	                			
	                    	validType:'other_remarks1'
	                	}}},
  				...
  				{field : 'write_off2',title : '核销方式',width : 80,align:'center',
  					formatter:function(val,row,index){
  						if(val==0 ){
  							return "打卡";
  						}else if(val==1){
  				...
  				{field : 'other_date',title : '核销日期',width : 80,align:'center'},
  					{field : 'people_other',title : '核销人',width : 80,align:'center'},
		    ...
		    onAfterEdit:function(index,row){//编辑关闭后
		    	...
		    	$('#dg').datagrid("endEdit",editval);
					$.ajax({
           			 type: "post",
          				url : 'payable.do?method=updaptez',
           				data :  row
		   ...
		   //双击事件
            onDblClickRow: function (rowIndex,rowData) {
				...
				if (row.shiping_order_num!='合计'&&row.write_off2 !=3) {
					return 'color:blue;';
				}
  	...
  	*/
		...
		for ( var i = 0; i < array.length; i++) {
			...
			if (id != "") {
				...
				title : '费用核销',
				url : 'payable.do?method=getEditCompanyPagez&pid=' + id,
				width : 500,
				height:300,
				buttons : [{
  ...
  function compute(colName) {
  	...
  	function searchCar_owner(){
  		$("#dg").datagrid('load',{
	 	 		...
	 	 		other_date:$.trim($("#other_date").val()),
	 	 		write_off2:$.trim($("#write_off2").val()),
	 	 		send_station:$.trim($("#send_station").val()),
	 	 		goods_name:$.trim($("#goods_name").val()),
	 	 		goods_num:$.trim($("#goods_num").val()),
	 	 		payble_other:$.trim($("#payble_other").val()),
	 	 		ss:$.trim($("#ss").val()),
	 	});	  
  ...
  function del() {
		...
		for ( var i = 0; i < array.length; i++) {
			...
			if (a== true) {
					...
					if (r) {
						$.ajax({
							url : 'payable.do?method=editOther1&pid=' + id,
							type : "POST",
							data : {
								id:id.join(",")
							},
    	...
    	<div region="north" title="检索" class="st-north" border="false" collapsible="false"  >
	    	<lable class="divlabel">订单编号:</lable>
	    	 ...
	    	 <lable class="divlabel">核销日期:</lable>
	    	 	<input  id="other_date"  class="Wdate"   style="width:100px;height:22px; " onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd '})" />
	    	 	 -
	    	 <input  id="ss"  class="Wdate"   style="width:100px;height:22px; " onfocus="WdatePicker({skin:'twoer',dateFmt:'yyyy-MM-dd '})" />
	    	 <lable class="divlabel">发货地:</lable>
	     ...
	     <a id="btnMoreSearch" class="btn btn-sm btn-link" type="button" style="margin-top:16px;" href="javascript:void(0)" state="close">更多条件</a>
	    	 <div id="searchInfoId" style="display: none">
				...
				<td>核销:</td>
					<td ><select id='write_off2'  name="write_off2"  style="width:50px;height:22px">
							<option> </option>
	    	 ...
	    	 <input type="text" class="search-text" id="goods_num"  style="width:100px"  >
	    	 	<lable class="divlabel">应付其他费:</lable>
	    	 <input type="text" class="search-text" id="payble_other"  style="width:100px"  >
