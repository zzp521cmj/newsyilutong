...
<html>
  ...
  var pid=[];
 	...
 	var submitFormEdit = function($dialog, $grid, $pjq, $mainMenu) {
 	 ...
 	 //可提交
		$pjq.messager.confirm('中转费结算','确定要提交吗?',function(r){   
	    	if (r){    
				$.ajax({
					type: "POST",
		  			url:'settleTC_fu.do?method=delSettlementYf',
		  			data:{
		  			dds:pid.join(","),
		  			...
		  			success:function(msg){
		  				if(msg.flag){
		  					$pjq.messager.alert('运单结算','运单中转结算提审成功','info');
			    ...
			    $pjq.messager.alert('运单结算', '请选择提审部门', 'info');
			  }
	};
