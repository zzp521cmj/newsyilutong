  ...
  <script type="text/javascript">
 		...
 		var did= '<%=user.getDid()%>';
  		...
  		$('#isdelivery').val(did);
			...
			$("#flag").combobox("setValue", "4");
  			...
  			grid=$("#dg").datagrid({
  			url : 'settlementYf.do?method=getSettlementYfSHInfo&isdelivery='+did,
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		rowStyler: function(index,row){
  			...
  			columns : [[
			  	...
			  	{field : 'settlyf_sh',title : '审核状态', width : 80,align :'center',sortable:'true',
  				...
  				{field : 'reality_sum',title : '实收运费总额',width : 100,align:'center'	/* sortable: true */},
  				...
  				{field : 'settlyf_th_remaks',title : '结算提审备注',width : 90,align:'center',sortable:'true'}, 
  				 {field : 'settlyf_time',title : '结算提审时间',width : 120,align:'center',sortable:'true',
  				formatter: function(value,row,index){
  				if(value!=undefined){
  				value=value.substr(0,19);
  				}
		  	...
		  	if(ids.length>0){
				...
				buttons : [{
				...
				},
				 {
  				...
  				$.messager.alert("运费结算","请选择审核中信息","info");
  		}	
  	}
  	//修改
  	function updateUser(){
	...
	var ids=[];
  		var row=$("#dg").datagrid('getSelections');
		  	...
		  	if(ids.length>0){
	  			dialog = parent.jy.modalDialog({
				title : '结算审核',
				url : 'settlementYf.do?method=shSettlement&pid=' + ids.join(","),
				width : 600,
				height:420,
				buttons : [{
					text : '<input type="button" class="btncss" value="通过"/>',
				...
				},
				 {
  				...
  				$.messager.alert("结算审核","请选择已提审信息","info");
  		}			  		
	...
	//查看审核详情
  		function OrderUpdates(ids){
  			dialog = parent.jy.modalDialog({
			title : '结算审核记录',
			url : 'settlementYf.do?method=settlementShRC&pid='+ids,
/* 				 buttons :'#addbuttons'
*/
			width : 725,
			height:550,
