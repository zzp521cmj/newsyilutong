...
<html>
  ...
  var submitFormEditss=function($dialog, $grid, $pjq, $mainMenu) {
		...
		$pjq.messager.confirm('运费结算', '确定要结算提交吗?', function(r) {
			if (r) {
				$.ajax({
					type : "POST",
					url : 'settleDFYF_fu.do?method=addSettlementYf',
					...
					success : function(msg) {		
						if (msg.flag) {
							$pjq.messager.alert('运费结算', '运费结算成功', 'info');
							$dialog.dialog('close');
			    ...
			    $pjq.messager.alert('运费结算', '必填信息不可为空', 'info');
			  }
	};	
