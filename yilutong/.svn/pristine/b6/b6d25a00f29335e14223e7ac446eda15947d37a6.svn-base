...
<script type="text/javascript">
		...
		console.log(id);
		$.ajax({
			url : 'Destination.do?method=getShippingDriver',
			data : {
				id : id
			},
			success : function(data) {
				data.destination_fee = parseFloat(data.destination_fee)
						- parseFloat(data.paid_derived_fee.split(",")[1]);						
				data.paid_fee = parseFloat(data.paid_derived_fee.split(",")[0])
						- parseFloat(data.paid_derived_fee.split(",")[1]);
	...
	var submitFormEdit = function($dialog, $grid, $pjq, $mainMenu) {
		if ($("#Regform").validationEngine('validate')
			...
			var destination_fee = $("#destination_fee").val();
			$pjq.messager.confirm('结算审核', '确定要审核吗?', function(r) {
				if (r) {
					$.ajax({
						type : "POST",
						url : 'Destination.do?method=UpdateCheckedShipp',
						data : {
							id : pid.join(","),
							paid_fee : paid_fee,
						...
						error : function() {
							$pjq.messager.alert('结算审核', '审核失败！', 'info');
						}
					});
				}
			});
		...
		if ($("#Regform").validationEngine('validate')
			...
			$pjq.messager.confirm('结算审核', '确定要驳回吗?', function(r) {
				if (r) {
					$.ajax({
						type : "POST",
						url : 'Destination.do?method=UpdateCheckedBohui',
						//data : $('#Regform').serialize(),
						data : {
							id : pid.join(","),
							paid_fee : paid_fee,
		...
		<table class="tableclass">
			...
			<tr>
				...
				<td class="td2"><input placeholder="0.00"
					...
					type="text" id="destination_fee" name="destination_fee" />
				</td>
			</tr>
			<tr>
				<td>实收代收运费：</td>
				<td class="td2"><input placeholder="0.00"
					class="validate[custom[number],max[99999.99],min[0.00],custom[dd]]"
					type="text" id="paid_fee" name="paid_fee" />
				</td>
				<td><font color="red" style="margin-right:10px">*</font>付款方式：</td>
				<td class="td2" colspan="3"><input type="radio" name="destination_stata"
					value="1" style="width:14px" onclick="xj(this)" /><label
					style="margin-left: 10px">现金</label> <input type="radio"
					name="destination_stata" value="2" style="width:14px" onclick="sk(this)" /><label
					style="margin-left: 10px">转账</label> <br> <input type="radio"
					name="destination_stata" value="3" style="width:14px" onclick="zp(this)" /><label
					style="margin-left: 10px">支票</label> <input type="radio"
					name="destination_stata" value="4" style="width:14px" onclick="xj(this)" /><label
					style="margin-left: 10px">其他</label>
				</td>
			</tr>
			<tr>
