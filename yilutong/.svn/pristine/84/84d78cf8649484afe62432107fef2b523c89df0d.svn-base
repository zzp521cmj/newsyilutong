  ...
  <script type="text/javascript">
 		...
 		var editval;
  		...
  		$('#dlg1').dialog({
			...
			$("#kname").val('<%=flg%>');
  			grid=$("#dg").datagrid({
  			url : 'settleDaisk.do?method=getSettlementYf&kname='+'<%=flg%>',
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		rowStyler: function(index,row){
				if (row.setldaisk_tz!=2||row.is_setldaisk==0||row.setldaisk_sh==3||(row.setldaisk_sh==2&&(parseFloat(row.trade_agency)!=parseFloat(row.setldaisk_trade)))){
					return 'color:blue;';
				}else if (row.setldaisk_sh==0){
					return 'color:rgb(221, 14, 211);';
				}else if(row.setldaisk_sh==1){
					return 'color:red;';
				}
			},
  			columns : [[
  				...
  				{field : 'goods_vol',title : '体积',width : 60,align:'center'},
  					{field : 'trade_agency',title : '应付代收货款',width : 100,align:'center'/* sortable: true */},
  				{field : 'setldaisk_trade',title : '实付代收货款',width : 100,align:'center'	/* sortable: true */},
  				{field : 'trade_cha',title : '差异',width : 70,align:'center',
  				/* formatter:function(val,row,index){			
		  					if(isNaN(parseFloat(row.setldaisk_trade))){
					  		 val=row.trade_agency;
					  		}else{
					  		 val=parseFloat(row.trade_agency)-parseFloat(row.setldaisk_trade); 	
					  		}	  		
  							return val;		
  					} */
  				...
  				{field : 'setldaisk_tz',title : '结算调整', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  							 if(val==1){ 						
  						return "调整未结算";
  						}else if(val==2){ 						
  					...
  					{field : 'setldaisk_sh',title : '是否审核', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  						if(val==0||val==null){
  							return "未审核";
  						}else if(val==1){ 						
  				...
  				{field : 'is_setldaisk',title : '结算方式', width : 80,align :'center',sortable:'true',
  				    formatter:function(val,row,index){
  							if(val==0){
  							return "未结算";
  						}else if(val==1){ 						
  	...
  	function settlementSh(){
  		...
  		var row=$("#dg").datagrid('getSelections');
  			if(row.length>0){
	  			...
	  			if(row[i].setldaisk_sh==0&&row[i].setldaisk_tz==2){   
		  	...
		  	if(ids.length>0){
				...
				title : '结算提审',
				url : 'settleDaisk.do?method=thSettlement&pid=' + ids.join(","),
				width :450,
				height:300,
				buttons : [{
		  ...
		  var wids=[];
		 	var row=$("#dg").datagrid('getSelections');
	  		...
	  		//&&(parseFloat(row[i].transport_pay)!=parseFloat(row[i].reality_sum)
		  			if(row[i].is_setldaisk==0||row[i].setldaisk_tz==1){
			  				ids.push(row[i].shiping_order_id);	  				
		  				}		  			
		  			}	
		  if(ids.length>0){
				  dialog= parent.jy.modalDialog({
					title : '代收款结算',
					url : 'settleDaisk.do?method=edSettlement&pid='+ids.join(","),
		/* 				 buttons :'#addbuttons'
		*/
					width : 530,
					height: 270,
	...
	//调整金额
		function changemoney(){
			 var ids="";
				var row=$("#dg").datagrid('getSelections');
		  			if(row.length==1){
			  			for(var i=0;i<row.length;i++){
			  			if(row[i].is_setldaisk==0||(row[i].setldaisk_tz!=2)||(row[i].setldaisk_sh==3)||(row[i].setldaisk_sh==2&&(parseFloat(row[i].trade_agency)!=parseFloat(row[i].setldaisk_trade)))){
			  				ids=row[i].shiping_order_id;	  				  								
			  				}	  		
				  		}
					if(ids!=""){
						$("#form").form('clear');			
						if(row[0].is_setldaisk==0||row[0].trade_cha==0){
			  					$("#yingshou").val(row[0].trade_agency);
			  				}else if(isNaN(parseFloat(row[0].setldaisk_trade))){
			  					$("#yingshou").val(row[0].trade_agency);	
			  				}else{
			  					$("#yingshou").val(parseFloat(row[0].trade_agency)-parseFloat(row[0].setldaisk_trade));
			  					}	  									
						$("#dlg1").dialog('open');
						}else{
		...
		function dealOrder(){
	  		var row=$("#dg").datagrid('getSelections');
	  		var reality_sum;	
	  		if(isNaN(parseFloat(row[0].setldaisk_trade))||(row[0].is_setldaisk==0&&row[0].setldaisk_tz==0)){
	  		 reality_sum=parseFloat($("#shishou").val());
	  		}else{
	  		 reality_sum=parseFloat($("#shishou").val())+parseFloat(row[0].setldaisk_trade);
	  		}	  		
				  			$.messager.confirm('确认','您确定要提交处理吗？',function(r){ 
			    			if (r){ 
			    				$.ajax({									
									type: "POST",
									url : 'settleDaisk.do?method=changePrice',
				           			data :  {
  		...
  		function OrderUpdates(ids){
  			dialog = parent.jy.modalDialog({
			title : '运费结算记录',
			url : 'settlementYf.do?method=setlRcordInfo&pid='+ids+'&state='+2,
/* 				 buttons :'#addbuttons'
*/
			width : 690,
			height:500,
  ...
  <body class="easyui-layout">
  	<form action="settleDaisk.do" id="outputform" method="post">
