...
<html>
  ...
  <script type="text/javascript">
	...
	$("#tbput").removeAttr("style");
  			...
  			grid=$("#dg").datagrid({
  			url : 'jmonitor.do?method=getJITMonitor&monitorType=3',
  			border : false,
  			rownumbers : true,
  			fit : true,
  			singleSelect : false,
  			...
  			columns : [[
  				...
  				{field : 'time_stay_real',title : '实际停留时间',width : 95,align:'center'},
  				{field : 'time_stay_plan',title : '计划停留时间',width : 95,align:'center'},
	...
	btnMoreSearch.click(function() {
  	...
  	*/
  	...
  	function searchmonitorRuning(){
  		...
  		//carId=$.trim($("#carid").select2("data").text);
  		//	}
    				...
    				$.ajax({
						...
						data:{del:del},
			  				success:function(data){
			  					if (data.flag) {
									parent.$.messager.alert("节点作业时间监控","节点作业时间监控处理成功",'info',
										function(r) {
											$('#dg').datagrid('reload');
 										});
  								} 
  ...
  <body class="easyui-layout">
	...
	<input type="hidden" name="method" value="outJITMonitor2">
